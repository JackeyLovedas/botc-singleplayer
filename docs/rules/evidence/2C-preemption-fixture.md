...
production=FROZEN_OUTSIDE_APPROVED_FIXTURE_IMPLEMENTATION
tests=FROZEN_IDENTITY_AND_BEHAVIOR
implementationAuthorized=false
```

This is Design Correction Round 2 of 2. No Round 3 is permitted.
...
```text
sourceUrls=[
  officialRulesExplanationOldid1310,
  officialGlossaryOldid2874,
  officialStatesOldid1039,
  officialSetupOldid1361,
  officialFlowergirlOldid2907,
  officialPitHagOldid2998,
  chineseNominationOldid5887,
  chineseVotingOldid5936,
  chineseExecutionOldid6420,
  officialNightsheetCommit3d6d930
]
sourceRevisionBinding=oldid/commit pinned; live bytes must match pinned bytes
sourceHashBinding=SHA256 table below; no future implementation SHA
```
...
| Official Flowergirl role page | oldid `2907`; 2025-09-24T08:47:04Z; fixed source checked 2026-08-21 | `abaf6621dfdfeec118db717026e6e2f8b21bae9c68d1d19184bd2407bf10d2d` | https://wiki.bloodontheclocktower.com/index.php?title=Flowergirl&amp;oldid=2907 |
| Official Pit-Hag role page | oldid `2998`; 2025-11-19T16:10:20Z; fixed source checked 2026-08-21 | `b8d242c72cfcdaacc7ee149535ce7c773aea6652ce24efb40d2bdce0a83c86cb` | https://wiki.bloodontheclocktower.com/index.php?title=Pit-Hag&amp;oldid=2998 |

The Flowergirl and Pit-Hag rows are now fixed to the rule-research revisions
and hashes supplied for this correction. The reviewer must re-fetch those
exact oldids and compare bytes; live pages may be used only as a reproducibility
check and may not silently replace the pinned revisions. The official
nightsheet remains canonical over the Chinese page where the latter presents
...
|---|---|---|---|---|---|---|---|---|
| 2C-R1 | exact role set is accepted | generator and assignment digest equal all 12 IDs | production setup/assignment witness | R1 | T1 | APPLICATION_COMMAND_INTEGRATION | PASS | SUP-2C-PREEMPTION-001 |
| 2C-R2 | first-night accepted paths remain historical | Philosopher/Seamstress provenance is unchanged | accepted replay comparison | R2 | T1 | LEGACY_REPLAY_COMPATIBILITY | PASS | SUP-2C-PREEMPTION-002 |
| 2C-R3 | day nomination/vote constraints hold | legal tally and deterministic negative outcomes | prospective/replay command audit | R3 | T1 | APPLICATION_COMMAND_INTEGRATION | PASS | SUP-2C-PREEMPTION-003 |
| 2C-R4 | Pit-Hag consequence preempts night tasks | fixed choice and causal death precede `NIGHT_TASKS` | bounded batch cross-link | R3 | T1 | APPLICATION_COMMAND_INTEGRATION | PASS | SUP-2C-PREEMPTION-002 |
| 2C-R5 | ordinary plan has two tasks | only generic kill and Flowergirl task are admitted | plan identity and unknown-task audit | R1 | T1 | STRUCTURAL_VALIDATION | PASS | SUP-2C-PREEMPTION-005 |
| 2C-R6 | dead Flowergirl cannot act | exactly one terminal `SOURCE_INELIGIBLE` settlement | prospective/replay settlement audit | R2 | T1 | LEGACY_REPLAY_COMPATIBILITY | PASS | SUP-2C-PREEMPTION-006 |
| 2C-R7 | complete loop is deterministic | A-R replays exactly and retry is idempotent | full stream/provenance audit | R4 | T1 | HOSTILE_REPLAY_REJECTION | PASS | SUP-2C-PREEMPTION-007 |
...
Only legal protocol reachability values are used: `R1` setup/plan application,
`R2` accepted-history replay, `R3` command/application integration, and `R4`
hostile replay rejection. `A-B`, `G-K`, and other step ranges are not values
for `ExpectedReachability`. Primary layers are limited to
`APPLICATION_COMMAND_INTEGRATION`, `LEGACY_REPLAY_COMPATIBILITY`,
`HOSTILE_REPLAY_REJECTION`, and `STRUCTURAL_VALIDATION`.

| CriterionId | RuleClaim | CompletionCriterion | RequiredEvidenceMechanism | ExpectedReachability | ExpectedTrust | ExpectedPrimaryLayer | ExpectedResult | SupportingAuthorityRequirement |
|---|---|---|---|---|---|---|---|---|
| 2C-A | exact setup | generator accepts the exact 12-role set | setup witness | R1 | T1 | APPLICATION_COMMAND_INTEGRATION | PASS | SUP-2C-PREEMPTION-001 |
| 2C-B | exact assignment | seat/role digest matches expected binding | assignment witness | R1 | T1 | APPLICATION_COMMAND_INTEGRATION | PASS | SUP-2C-PREEMPTION-001 |
| 2C-C | first-night compatibility | accepted plan opens unchanged | replay | R2 | T1 | LEGACY_REPLAY_COMPATIBILITY | PASS | SUP-2C-PREEMPTION-002 |
| 2C-D | Philosopher/Artist spent | choice/spend provenance settles once | ability evidence | R2 | T1 | LEGACY_REPLAY_COMPATIBILITY | PASS | SUP-2C-PREEMPTION-002 |
| 2C-E | Seamstress pair spent | Mutant/Savant provenance settles once | ability evidence | R2 | T1 | LEGACY_REPLAY_COMPATIBILITY | PASS | SUP-2C-PREEMPTION-002 |
| 2C-F | dawn boundary | first-night close is exact | transition audit | R2 | T1 | LEGACY_REPLAY_COMPATIBILITY | PASS | SUP-2C-PREEMPTION-002 |
| 2C-G | day opening | day revision/window is exact | phase audit | R3 | T1 | APPLICATION_COMMAND_INTEGRATION | PASS | SUP-2C-PREEMPTION-003 |
| 2C-H | nomination rule | one legal nomination is accepted | prospective audit | R3 | T1 | APPLICATION_COMMAND_INTEGRATION | PASS | SUP-2C-PREEMPTION-003 |
| 2C-I | vote provenance | voter/nomination/day facts are linked | event audit | R3 | T1 | APPLICATION_COMMAND_INTEGRATION | PASS | SUP-2C-PREEMPTION-003 |
| 2C-J | strict tally | threshold/nonzero/highest result is deterministic | tally negatives | R3 | T1 | APPLICATION_COMMAND_INTEGRATION | PASS | SUP-2C-PREEMPTION-003 |
| 2C-K | execution boundary | one daily execution, no death inference | replay batch | R4 | T1 | HOSTILE_REPLAY_REJECTION | PASS | SUP-2C-PREEMPTION-004 |
| 2C-L | Pit-Hag preemption | fixed action precedes `NIGHT_TASKS` | causal batch | R3 | T1 | APPLICATION_COMMAND_INTEGRATION | PASS | SUP-2C-PREEMPTION-002 |
| 2C-M | causal death | Pit-Hag cause is explicit and distinct | death cross-link | R3 | T1 | APPLICATION_COMMAND_INTEGRATION | PASS | SUP-2C-PREEMPTION-002 |
| 2C-N | transition order | only validated preemption enters night tasks | reorder negatives | R4 | T1 | HOSTILE_REPLAY_REJECTION | PASS | SUP-2C-PREEMPTION-004 |
| 2C-O | exact ordinary plan | only generic kill and Flowergirl task | plan audit | R1 | T1 | STRUCTURAL_VALIDATION | PASS | SUP-2C-PREEMPTION-005 |
| 2C-P | Vortox target | Flowergirl dies before her task | target/death witness | R3 | T1 | APPLICATION_COMMAND_INTEGRATION | PASS | SUP-2C-PREEMPTION-006 |
| 2C-Q | source ineligible | one Flowergirl terminal settlement only | task replay audit | R2 | T1 | LEGACY_REPLAY_COMPATIBILITY | PASS | SUP-2C-PREEMPTION-006 |
| 2C-R | replay/idempotency | full stream replays and retries exactly | stream manifest | R4 | T1 | HOSTILE_REPLAY_REJECTION | PASS | SUP-2C-PREEMPTION-007 |

### Planned supporting-authority ledger

The following are planned supporting records, not primary identities. Each is
materialized once at implementation and remains subordinate to its listed
criteria:

| AuthorityId | Status | UsedByCriteria | Mechanism | Primary |
|---|---|---|---|---|
| SUP-2C-PREEMPTION-001 | PLANNED | 2C-A,2C-B,2C-O | setup/assignment/plan manifest | false |
| SUP-2C-PREEMPTION-002 | PLANNED | 2C-R2,2C-C,2C-D,2C-E,2C-L,2C-M | accepted replay and causal chain | false |
| SUP-2C-PREEMPTION-003 | PLANNED | 2C-R3,2C-G,2C-H,2C-I,2C-J | command/tally evidence | false |
| SUP-2C-PREEMPTION-004 | PLANNED | 2C-R4,2C-K,2C-N | hostile replay rejection | false |
| SUP-2C-PREEMPTION-005 | PLANNED | 2C-R5,2C-O | exact ordinary-plan manifest | false |
| SUP-2C-PREEMPTION-006 | PLANNED | 2C-R6,2C-P,2C-Q | Flowergirl death/settlement link | false |
| SUP-2C-PREEMPTION-007 | PLANNED | 2C-R7,2C-R | full stream/retry manifest | false |

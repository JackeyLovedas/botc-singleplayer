...
| C13-C14, C30 | `packages/domain-core/src/rebuild.test.ts` | V3 restart replay from the application-verified accepted-stream fixture; cloned-fixture hostile batch mutations; legacy subsystem regression |
...
| S02 | `packages/domain-core/src/rebuild.test.ts` | Source/task/player/seat/role/revision/tenure/ability/catalog/opportunity cross-links mutated only after the application-verified accepted-stream fixture is cloned |
...
Repair Round 1 local result: focused `10 files / 662 tests PASS`; typecheck and full lint `PASS`; ordinary and single-fork coverage `34 files / 1456 tests PASS`; single-fork duration `136.44s`; coverage `87.18 / 82.00 / 97.75`; no `onTaskUpdate` failure. Repair publication, fresh exact-head CI, and independent final review remain pending.

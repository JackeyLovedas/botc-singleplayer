# Phase 3 Slice 2B19A3A — Effective Base Dreamer + Effective Vortox Forced-False Information Design Round 3
...
- parentRound2Design: `docs/implementation/phase-3-slice-2b19a3a-design-round-2.md`
- parentRound2DesignSha256: `a3059ff3d3bd9011df19660123139fbbd890a8da549a00c07ac09a65db04a172`
- round2Review: `docs/implementation/phase-3-slice-2b19a3a-design-review-round-2.md`
- round2ReviewSha256: `51defb79e2df640f666fb4a702668a5652678a31b590526e79afddf38c3ad8d1`
- round2ReviewVerdict: `RULE_DESIGN_FIX_REQUIRED`
- round2OnlyBlocker: `R3_ACCEPTED_STREAM_PRIMARY_LAYER_MISCLASSIFICATION_C27_C32`
...
- ruleEvidenceSha256: `7c427b38240ef888e7b9851b5c8ce9a67377224722be01fa8f3154e5f731a7eb`
- preCorrectionRuleEvidenceSha256: `2da6b7c9d6fea31bbab05674ac3e6a45213257c7af314f66d47eb4a8436f84b6`
- ruleEvidenceResponsibilityOnlyChanged: `true`
- ruleReady: `true`
...
- classificationCorrectionOnly: `true`
...
This document is the complete standalone Round 3 design and sole candidate implementation authority. It is not an erratum, addendum, or delta. It inherits every Round 2 behavior, type, exact-shape, nine-or-ten-entry ledger, batch, projection, receipt, test, and allowlist contract. It changes only Reachability, Trust, PrimaryLayer, and supporting-authority classifications, closes only `R3_ACCEPTED_STREAM_PRIMARY_LAYER_MISCLASSIFICATION_C27_C32`, and authorizes only independent Round 3 rule-design review. No production/test edit, feature commit, push, PR, merge, later Slice, or coverage promotion is authorized until this exact materialized document receives `RULE_DESIGN_PASS`.
...
### classificationAuthorityRound3

- Only a real legal successful full `GameApplicationService command -> producer -> accepted events -> receipt -> append -> rebuild -> projection` path whose success is the main assertion may be `R1 + ACCEPTED_STREAM_INTEGRATION`.
- An accepted prefix followed by a manually tampered event, payload, history, provenance, tenure, impairment, delivery, or ledger whose main assertion is replay, rebuild, or prospective rejection is `R3 + HOSTILE_REPLAY_REJECTION`; the accepted prefix is supporting only.
- Direct exact shape, version, ID, field, application-boundary, or commit-boundary validation whose main assertion is rejection or failure is `R3 + STRUCTURAL_VALIDATION`.
- A deterministic private pure policy is `T3 + PURE_POLICY_SEAM`.
- Accepted-prefix setup never promotes a hostile, private, or fault-injected rejection to accepted integration.
- If a claim cannot honestly fit one of the seven frozen layers without new architecture or trust layer, return `RESLICE_REQUIRED`.

The actual C27-C32 mechanisms are frozen exactly:

- C27 starts from a real accepted V3-open prefix, then manually hostile cloned canonical state/provenance makes capability unresolved; the direct private application preflight/boundary rejects with the exact dependency failure, no receipt, zero events, and OPEN opportunity. It is `R3 / T1 / STRUCTURAL_VALIDATION`; the accepted prefix is supporting only.
- C28 starts from a real accepted V3-open prefix, then a manually substituted catalog forces candidate shortage; the direct application boundary rejects with the exact dependency failure, no receipt, zero events, and OPEN opportunity. It is `R3 / T1 / STRUCTURAL_VALIDATION`; the accepted prefix is supporting only.
- C29 starts from a real accepted prefix, then monkeypatches private `createBatch` to mutate the proposed delivery/batch so prospective validation returns a `DomainError`; the main assertion is the rejection and stage. It is `R3 / T1 / HOSTILE_REPLAY_REJECTION`; the accepted prefix is supporting only.
- C30 starts from a real accepted prefix, then monkeypatches the proposed batch, payload, or proxy so an unexpected non-`DomainError` occurs during prospective validation; the main assertion is the rejection and stage. It is `R3 / T1 / HOSTILE_REPLAY_REJECTION`; the accepted prefix is supporting only.
- C31 uses the real service/command plus a fault-injecting ID/clock metadata adapter; the main assertion is the metadata-boundary failure, code, stage, no receipt, zero events, and OPEN opportunity. It is `R3 / T1 / STRUCTURAL_VALIDATION`; the accepted prefix is supporting only.
- C32 uses the real service/command plus commit-store `failBeforeCommit` or `failDuringCommit`; the main assertion is the commit-boundary failure, code, stage, no receipt, zero accepted events, and OPEN opportunity. It is `R3 / T1 / STRUCTURAL_VALIDATION`; the accepted prefix is supporting only.
- C33 remains unchanged at `R1 / T1 / ACCEPTED_STREAM_INTEGRATION` as the real recovery success.
...
| C27 | RC-C27 unresolved capability is receipt-free | R3 | T1 | STRUCTURAL_VALIDATION | `packages/application/src/game-application-service.test.ts :: 2B19A3A-C27` | `packages/application/src/game-application-service.test.ts :: 2B19A3A-C01 (accepted-prefix fixture only; non-primary)` | exact dependency failure/no receipt/zero events/OPEN |
| C28 | RC-C28 candidate failure is receipt-free | R3 | T1 | STRUCTURAL_VALIDATION | `packages/application/src/game-application-service.test.ts :: 2B19A3A-C28` | `packages/application/src/game-application-service.test.ts :: 2B19A3A-C01 (accepted-prefix fixture only; non-primary)` | exact dependency failure/no receipt/zero events/OPEN |
| C29 | RC-C29 prospective DomainError preserves its stage | R3 | T1 | HOSTILE_REPLAY_REJECTION | `packages/application/src/game-application-service.test.ts :: 2B19A3A-C29` | `packages/application/src/game-application-service.test.ts :: 2B19A3A-C01 (accepted-prefix fixture only; non-primary)` | first-night-role-action/no receipt |
| C30 | RC-C30 unexpected prospective failure preserves its stage | R3 | T1 | HOSTILE_REPLAY_REJECTION | `packages/application/src/game-application-service.test.ts :: 2B19A3A-C30` | `packages/application/src/game-application-service.test.ts :: 2B19A3A-C01 (accepted-prefix fixture only; non-primary)` | prospective-validation/no receipt |
| C31 | RC-C31 metadata failure preserves its code and stage | R3 | T1 | STRUCTURAL_VALIDATION | `packages/application/src/game-application-service.test.ts :: 2B19A3A-C31` | `packages/application/src/game-application-service.test.ts :: 2B19A3A-C01 (accepted-prefix fixture only; non-primary)` | MetadataGenerationFailed/event-metadata/no receipt |
| C32 | RC-C32 append failure preserves its code and stage | R3 | T1 | STRUCTURAL_VALIDATION | `packages/application/src/game-application-service.test.ts :: 2B19A3A-C32` | `packages/application/src/game-application-service.test.ts :: 2B19A3A-C01 (accepted-prefix fixture only; non-primary)` | EventStoreAppendFailed/accepted-commit/no receipt |
...
| S37 | RC-S37 accepted-stream integration requires R1 legal full-path success as the main assertion | R3 | T1 | STRUCTURAL_VALIDATION | `STATIC_REACHABILITY_PRIMARY_LAYER_INDEPENDENCE_AUDIT` | NONE | reject every ACCEPTED_STREAM_INTEGRATION row whose R is not R1 and reject every manually mutated, private-seam, or fault-injected rejection as accepted-stream authority |
...
`STATIC_TRACEABILITY_SCHEMA_AUDIT` parses both tables and requires unique ID, unique `RC-*`, exactly one of R1-R4, exactly one of T1-T3, exactly one of the seven tokens, nonempty primaryTest, explicit supportingTests, and nonempty expectedResult. `STATIC_PRIMARY_LAYER_TOKEN_AUDIT` rejects `/`, `+`, comma-separated, free-form, and unknown layer values. `STATIC_REACHABILITY_PRIMARY_LAYER_INDEPENDENCE_AUDIT` independently requires the real legal successful full `GameApplicationService command -> producer -> accepted events -> receipt -> append -> rebuild -> projection` chain for every `ACCEPTED_STREAM_INTEGRATION` primary authority, rejects every such row whose reachability is not R1, and forbids accepted-prefix setup from promoting a manually mutated, private-seam, or fault-injected rejection test to accepted-stream authority. These audits may be implemented as deterministic review commands; they do not authorize a new production or support script.
...
- `docs/implementation/phase-3-slice-2b19a3a-design-review-round-3.md`
...
Governance, Round 1 design/review, and Round 2 design/review are immutable. Evidence changed only by the exact `NON_AUTHORITATIVE_ENGINEERING_TRACE_REFERENCE` responsibility marker; its external rule and source bytes remain unchanged. `docs/PROJECT_STATE.md` is forbidden. PR body/comments are GitHub records, not local path exceptions. Post-merge review archives are a separate docs-only closeout and are not authorized in the feature diff.
...
1. Exact Round 3 materialization receives independent `RULE_DESIGN_PASS` before production/test edits.
...
23. C27, C28, C31, and C32 are `STRUCTURAL_VALIDATION`.
24. C29 and C30 are `HOSTILE_REPLAY_REJECTION`.
25. The accepted prefix for C27-C32 is supporting only and never primary accepted-stream authority.
26. C33 remains `R1 / T1 / ACCEPTED_STREAM_INTEGRATION` as the real recovery success.
27. The evidence responsibility marker separates external rule claims from Round 3 engineering classification authority.
28. No other trace-row classification or behavior contract changed from Round 2.
29. Diff is a subset of the closed path set; baseline harness is unchanged; at most three named new support files exist.
30. Production diff stays within exact five and `<=1000` added LOC.
31. No forbidden surface/path changes.
32. Targeted checks and all full gates pass.
33. Exact frozen HEAD passes all three CI claims.
34. Complete final review returns `CODE_REVIEW_PASS`, `RULE_REVIEW_PASS`, `remainingBlockers=[]` for exact PR HEAD.
35. Both verbatim audit comments are re-read and match PR HEAD before merge.
36. Worktree is clean, local/remote/PR HEADs match, and no later Slice started.
...
The user hard stop above eight production files or 1500 LOC is not permission to exceed this stricter design ceiling. No Design Round 4 exists. A remaining blocker after Round 3 is `RESLICE_REQUIRED` unless a substantive rule conflict or source unavailability requires `HUMAN_BLOCKED`.

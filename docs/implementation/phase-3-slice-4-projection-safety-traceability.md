...
| S4-C05 | `packages/projections/src/public-player-projection.test.ts` | associates votes and counts with each public nomination ordinal | PROJECTION | R1_CURRENTLY_REACHABLE_APPLICATION_PATH | T2_CANONICAL_DERIVED_STATE | NONE | PASS |
...
| S4-C07 | `packages/projections/src/public-player-projection.test.ts` | rejects a mutated accepted prefix before projection | HOSTILE_REPLAY_REJECTION | R3_HOSTILE_OR_CORRUPTED_HISTORY | T1_EXTERNAL_OR_PERSISTED_BOUNDARY | NONE | PASS |
...
an empty-stream precondition. S4-C05 is intentionally T2 because its canonical
state fixture exercises the derived nomination/vote projection directly; S4-C01
separately covers accepted-stream replay parity. The

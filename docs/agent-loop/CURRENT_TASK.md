...
- The A3B1 twelve-field frozen snapshot is refreshed from the actual `851743b` candidate bytes: six semantic tests and six authority markers; project executions `6/6`; project/current inventory `9d8726005537db396683c3701546a85f0094b3e84ca062f1d7113a66b3eef189`; semantic inventory `bd194c778f83c42c4bc46307f028e1a289b01c50a49c2169ce2a07c267a317f4`; authority inventory `c42fc09726d54c1e9ea6f7d88756435340f7e329cd5fd45f00c9030979e574c6`. Non-marker, physical-file, traceability, dynamic-row, support, marker, owner, criterion, and prefix values remain unchanged; the A3A record remains byte-identical.
- Registry load and self-test `22/22` pass. The formal verifier over the real `851743b` candidate reports 1499 tests, A3B1 `6` semantic owner tests, `60/60` traceability rows, `58` dynamic rows, four support authorities, and zero missing, duplicate, unexpected, ambiguous, or wrong-owner results.
- Repair remains `1/2`; F01-F03 are closed and the sole review blocker remains `F-04_PR_BODY_STALE_AFTER_EXACT_HEAD_CI`, pending the registry-refresh commit, fresh repaired-head profile, PR-body reconciliation, exact-head CI, and fresh independent review. The preserved review remains bound only to pre-repair HEAD `17472c7d2ac3d7bd52f5a9f0713fcd94b2f5f78d`.
...
- ACTIVE ownership contract `2B19A3B1` uses marker `[2B19A3B1-`, owner `application-service-dreamer-vortox`, and support prefix `SUP-2B19A3B1-`. Its refreshed actual baseline records six semantic application tests only in the owner project; the canonical parser resolves all 60 rows after the projection-marker microfix. Accepted A3A frozen values and the prior coverage profile remain unchanged; fresh profiling must supersede the historical profile.
...
- The only remaining blocker is `F-04_PR_BODY_STALE_AFTER_EXACT_HEAD_CI`; merge remains prohibited pending the registry-refresh commit, fresh repaired-head profile, PR-body reconciliation, new exact-head CI, and fresh complete independent review.
...
- Recovery is complete and cannot be repeated. Coverage profile evidence is `docs/implementation/phase-3-slice-2b19a3b1-coverage-profile.md`; the accepted profile remains immutable. The next gate is the registry-refresh commit followed by fresh repaired-head profiling, PR-body reconciliation, new exact-head CI, and a fresh complete independent review.

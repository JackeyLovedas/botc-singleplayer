...
describeApplicationServiceShard("core", "GameApplicationService", () => {
  ...
  it("fails gained Dreamer V2 opening receipt-free and opens base Snake Charmer opportunities", async () => {
    ...
    expect(gainedFirst).toStrictEqual({
      status: "failed",
      gameId: ids.game,
      code: "ApplicationNotConfigured",
    ...
    expect(gainedSecond).toStrictEqual(gainedFirst);
    expect(await dreamerStore.loadDomainEvents(ids.game)).toStrictEqual(gainedBeforeEvents);
    expect(dreamerStore.getReceiptCount()).toBe(gainedBeforeReceipts);
...
describeApplicationServiceShard("information-and-later-actions", "GameApplicationService", () => {
  ...
  it("[2B19A2-C15] fails a represented DRUNK base Dreamer receipt-free through the real Philosopher chain", async () => {
    const { service, commandStore } = makeService();
  ...
  it("[2B19A2-C17] fails an effective current Vortox receipt-free", async () => {
    const { service, commandStore } = makeService();
  ...
  it("[2B19A2-C20] keeps every retryable unsupported or dependency path receipt-free and mutation-free", async () => {
    ...
    for (const [name, roles] of [
      ["vortox", noPhilosopherVortoxExactRoleIds],
      ["no-dashii", noPhilosopherNoDashiiExactRoleIds]
    ] as const) {

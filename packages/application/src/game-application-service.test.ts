...
describeApplicationServiceShard("core", "F06 first-night completion to ordinary-night application chain", () => {
  it("accepts CompleteNight and reaches day, vote, execution, and ordinary-night planning", async () => {
    ...
    expectAcceptedResult(await service.execute(assignCharactersCommand()));
    expectAcceptedResult(await service.execute(initializeFirstNightCommand()));
    expectAcceptedResult(await service.execute(planFirstNightTasksCommand()));
    ...
    for (let index = 0; planned?.firstNightTaskPlan !== undefined; index += 1) {
      ...
      if (next.source.kind === "SYSTEM") {
        const settled = await service.execute(settleFirstNightSystemTaskCommand({
          commandId: commandId(`f06-settle-system-${index}`), expectedGameVersion: planned.gameVersion,
        ...
        if (openState === undefined || opportunity === undefined) throw new Error("Expected F06 role opportunity");
        const settled = next.taskType === "PHILOSOPHER_ACTION"
          ? await service.execute(submitPhilosopherActionCommand({
              commandId: commandId(`f06-settle-role-${index}`), expectedGameVersion: openState.gameVersion,
              payload: { commandType: "SubmitPhilosopherAction", taskId: next.taskId, opportunityId: opportunity.opportunityId,
                decision: { kind: "CHOOSE_GOOD_CHARACTER", roleId: roleId("artist") } }
            }))
          : next.taskType === "SEAMSTRESS_ACTION"
          ? await service.execute(submitSeamstressActionCommand({
              commandId: commandId(`f06-settle-role-${index}`), expectedGameVersion: openState.gameVersion,
              payload: { commandType: "SubmitSeamstressAction", taskId: next.taskId, opportunityId: opportunity.opportunityId,
                decision: { kind: "CHOOSE_TWO_PLAYERS", targetPlayerIds: openState.currentCharacterState!.entries
                  .filter((entry) => entry.playerId !== opportunity.sourcePlayerId).slice(0, 2).map((entry) => entry.playerId) as [typeof opportunity.sourcePlayerId, typeof opportunity.sourcePlayerId] } }
            }))
          : await service.execute(submitWitchActionCommand({
              ...
              payload: { commandType: "SubmitWitchAction", taskId: next.taskId, opportunityId: opportunity.opportunityId,
                decision: { kind: "CHOOSE_PLAYER", targetPlayerId: openState.currentCharacterState!.entries.find((entry) => entry.playerId !== opportunity.sourcePlayerId)!.playerId } }
            }));
        if (next.taskType === "SEAMSTRESS_ACTION") expectEventSummaryAcceptedResult(settled);
        else expectAcceptedResult(settled);
      }
      planned = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
    }
    const ready = planned;
    const complete = await service.execute(completeNightCommand({
      commandId: commandId("f06-complete-first-night"), expectedGameVersion: ready!.gameVersion,
      payload: { commandType: "CompleteNight", phase: "FIRST_NIGHT", planVersion: ready!.firstNightTaskPlan!.taskPlanVersion, nightNumber: 1 }
    }));
    expectAcceptedResult(complete);
    expect(complete.events.map((event) => event.eventType)).toStrictEqual(["PhaseTransitioned"]);
    expect(complete.events[0]).toMatchObject({ payload: { fromPhase: "FIRST_NIGHT", toPhase: "DAWN_RESOLUTION", transitionReason: "FIRST_NIGHT_COMPLETED" } });
    const afterComplete = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
    expect(afterComplete?.phase).toBe("DAWN_RESOLUTION");

    const tampered = structuredClone(await commandStore.loadDomainEvents(ids.game)).map((event) =>
      event.eventType === "PhaseTransitioned" && event.payload.transitionReason === "FIRST_NIGHT_COMPLETED"
        ? { ...event, payload: { ...event.payload, nightNumberAfter: 2 } }
        : event
    );
    expect(() => rebuildOptionalGameState(tampered)).toThrow();

    const dawn = await service.execute(publishDawnCommand({ expectedGameVersion: afterComplete!.gameVersion }));
    expectAcceptedResult(dawn);
    const afterDawn = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
    expect(afterDawn?.phase).toBe("DAY_DISCUSSION");
    const openNominations = await service.execute(openNominationsCommand({ expectedGameVersion: afterDawn!.gameVersion }));
    expectAcceptedResult(openNominations);
    ...
    const afterNomination = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
    const nominationId = afterNomination!.nominations!.at(-1)!.nominationId;
    const openVote = await service.execute(openVoteCommand({
      commandId: commandId("f06-open-vote"), expectedGameVersion: afterNomination!.gameVersion,
      payload: { commandType: "OpenVote", nominationId }
    }));
    expectAcceptedResult(openVote);
    let afterOpenVote = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
    for (const [index, voter] of afterOpenVote!.roster!.entries.slice(0, 6).entries()) {
      const vote = await service.execute(castVoteCommand({
        commandId: commandId(`f06-cast-vote-${index + 1}`), expectedGameVersion: afterOpenVote!.gameVersion,
        actor: { kind: "ai", playerId: voter.playerId }, payload: { commandType: "CastVote", nominationId, choice: "YES" }
      }));
      expectAcceptedResult(vote);
      afterOpenVote = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
    }
    const completeVote = await service.execute(completeVoteCommand({
      commandId: commandId("f06-complete-vote"), expectedGameVersion: afterOpenVote!.gameVersion,
      payload: { commandType: "CompleteVote", nominationId }
    }));
    ...
    expect(afterClose?.phase).toBe("EXECUTION_RESOLUTION");
    const resolve = await service.execute(resolveExecutionCommand({
      commandId: commandId("f06-resolve-execution"), expectedGameVersion: afterClose!.gameVersion,
      payload: { commandType: "ResolveExecution", blockId: "block-v1:1:1" }
    }));
...
type Slice3DayFixture = {
  readonly service: GameApplicationService;
  readonly commandStore: MemoryCommandCommitStore;
  readonly state: GameState;
  readonly deadPlayerId: ReturnType<typeof playerId>;
};

const reachSlice3SecondDay = async (prefix: string): Promise<Slice3DayFixture> => {
  const { service, commandStore } = makeService();
  const exactRoleIds = [
    "clockmaker", "flowergirl", "savant", "seamstress", "philosopher", "artist",
    "sage", "mutant", "klutz", "evil_twin", "pit_hag", "vortox"
  ].map(roleId);
  expectAcceptedResult(await service.execute(createGameCommand({ payload: { ...createGameCommand().payload, rootSeed: "2c-preemption-pithag-vortox-v1" } })));
  expectAcceptedResult(await service.execute(selectScriptCommand()));
  expectAcceptedResult(await service.execute(generateSetupCommand({ payload: { commandType: "GenerateSetup", constraints: { exactRoleIds } } })));
  expectAcceptedResult(await service.execute(createPlayerRosterCommand()));
  expectAcceptedResult(await service.execute(assignCharactersCommand()));
  ...
  let state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  for (let index = 0; state?.firstNightTaskPlan !== undefined; index += 1) {
    const next = state.firstNightTaskPlan.tasks[state.firstNightTaskProgress?.settlements.length ?? 0];
    if (next === undefined) break;
    if (next.source.kind === "SYSTEM") {
      expectAcceptedResult(await service.execute(settleFirstNightSystemTaskCommand({
        commandId: commandId(`${prefix}-system-${index}`), expectedGameVersion: state.gameVersion,
        payload: { commandType: "SettleFirstNightSystemTask", taskId: next.taskId }
      })));
    } else if (next.taskType === "EVIL_TWIN_SETUP") {
      expectAcceptedResult(await service.execute(settleEvilTwinSetupCommand({
        commandId: commandId(`${prefix}-evil-twin-${index}`), expectedGameVersion: state.gameVersion,
        payload: { commandType: "SettleEvilTwinSetup", taskId: next.taskId }
      })));
    } else if (next.taskType === "CLOCKMAKER_INFORMATION") {
      expectEventSummaryAcceptedResult(await service.execute(settleClockmakerCommand(state, next.taskId, {
        commandId: commandId(`${prefix}-clockmaker-${index}`)
      })));
    } else {
      const opened = await service.execute(openFirstNightRoleActionOpportunityCommand({
        commandId: commandId(`${prefix}-open-${index}`), expectedGameVersion: state.gameVersion,
        payload: { commandType: "OpenFirstNightRoleActionOpportunity", taskId: next.taskId }
      }));
      expectAcceptedResult(opened);
      const openState = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
      const opportunity = openState?.firstNightActionOpportunities?.opportunities.find((entry) => entry.taskId === next.taskId);
      if (openState === undefined || opportunity === undefined) throw new Error("Expected Slice3 role opportunity");
      const settled = next.taskType === "PHILOSOPHER_ACTION"
        ? await service.execute(submitPhilosopherActionCommand({
            commandId: commandId(`${prefix}-philosopher-${index}`), expectedGameVersion: openState.gameVersion,
            payload: { commandType: "SubmitPhilosopherAction", taskId: next.taskId, opportunityId: opportunity.opportunityId,
              decision: { kind: "CHOOSE_GOOD_CHARACTER", roleId: roleId("artist") } }
          }))
        : await service.execute(submitSeamstressActionCommand({
            commandId: commandId(`${prefix}-seamstress-${index}`), expectedGameVersion: openState.gameVersion,
            payload: { commandType: "SubmitSeamstressAction", taskId: next.taskId, opportunityId: opportunity.opportunityId,
              decision: { kind: "CHOOSE_TWO_PLAYERS", targetPlayerIds: openState.currentCharacterState!.entries
                .filter((entry) => entry.playerId !== opportunity.sourcePlayerId).slice(0, 2).map((entry) => entry.playerId) as [ReturnType<typeof playerId>, ReturnType<typeof playerId>] } }
          }));
      if (next.taskType === "SEAMSTRESS_ACTION") expectEventSummaryAcceptedResult(settled);
      else expectAcceptedResult(settled);
    }
    state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  }
  if (state === undefined || state.firstNightTaskPlan === undefined) throw new Error("Expected settled first night");
  expectAcceptedResult(await service.execute(completeNightCommand({
    commandId: commandId(`${prefix}-complete-first-night`), expectedGameVersion: state.gameVersion,
    payload: { commandType: "CompleteNight", phase: "FIRST_NIGHT", planVersion: state.firstNightTaskPlan.taskPlanVersion, nightNumber: 1 }
  })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  if (state === undefined) throw new Error("Expected dawn state");
  expectAcceptedResult(await service.execute(publishDawnCommand({ commandId: commandId(`${prefix}-dawn-1`), expectedGameVersion: state.gameVersion })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  expectAcceptedResult(await service.execute(openNominationsCommand({ commandId: commandId(`${prefix}-open-nominations-1`), expectedGameVersion: state!.gameVersion })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  if (state === undefined || state.roster === undefined || state.assignment === undefined) throw new Error("Expected first nomination state");
  const deadPlayerId = state.assignment.assignments.find((entry) => entry.role.roleId === "pit_hag")?.playerId;
  const nominator = state.roster.entries.find((entry) => entry.playerId !== deadPlayerId);
  if (deadPlayerId === undefined || nominator === undefined) throw new Error("Expected execution target and nominator");
  expectAcceptedResult(await service.execute(declareNominationCommand({
    commandId: commandId(`${prefix}-nomination-1`), expectedGameVersion: state.gameVersion,
    actor: { kind: "ai", playerId: nominator.playerId }, payload: { commandType: "DeclareNomination", targetPlayerId: deadPlayerId }
  })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  const nominationId = state!.nominations!.at(-1)!.nominationId;
  expectAcceptedResult(await service.execute(openVoteCommand({ commandId: commandId(`${prefix}-open-vote-1`), expectedGameVersion: state!.gameVersion, payload: { commandType: "OpenVote", nominationId } })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  for (const [index, voter] of state!.roster!.entries.slice(0, 6).entries()) {
    expectAcceptedResult(await service.execute(castVoteCommand({
      commandId: commandId(`${prefix}-vote-${index}`), expectedGameVersion: state!.gameVersion,
      actor: { kind: "ai", playerId: voter.playerId }, payload: { commandType: "CastVote", nominationId, choice: "YES" }
    })));
    state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  }
  expectAcceptedResult(await service.execute(completeVoteCommand({ commandId: commandId(`${prefix}-complete-vote-1`), expectedGameVersion: state!.gameVersion, payload: { commandType: "CompleteVote", nominationId } })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  expectAcceptedResult(await service.execute(closeNominationsCommand({ commandId: commandId(`${prefix}-close-1`), expectedGameVersion: state!.gameVersion })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  expectAcceptedResult(await service.execute(resolveExecutionCommand({ commandId: commandId(`${prefix}-resolve-1`), expectedGameVersion: state!.gameVersion, payload: { commandType: "ResolveExecution", blockId: "block-v1:1:1" } })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  expectAcceptedResult(await service.execute(beginNightCommand({ commandId: commandId(`${prefix}-begin-ordinary`), expectedGameVersion: state!.gameVersion })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  const genericTask = state!.ordinaryNightTaskPlan!.tasks.find((task) => task.taskType === "GENERIC_DEMON_KILL")!;
  const flowergirlTask = state!.ordinaryNightTaskPlan!.tasks.find((task) => task.taskType === "FLOWERGIRL_ACTION")!;
  expectAcceptedResult(await service.execute(settleOrdinaryNightTaskCommand({ commandId: commandId(`${prefix}-settle-generic`), expectedGameVersion: state!.gameVersion, payload: { commandType: "SettleOrdinaryNightTask", taskId: genericTask.taskId } })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  expectAcceptedResult(await service.execute(settleOrdinaryNightTaskCommand({ commandId: commandId(`${prefix}-settle-flowergirl`), expectedGameVersion: state!.gameVersion, payload: { commandType: "SettleOrdinaryNightTask", taskId: flowergirlTask.taskId } })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  expectAcceptedResult(await service.execute(completeNightCommand({ commandId: commandId(`${prefix}-complete-ordinary`), expectedGameVersion: state!.gameVersion, payload: { commandType: "CompleteNight", phase: "NIGHT_TASKS", planVersion: state!.ordinaryNightTaskPlan!.planVersion, nightNumber: 2 } })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  expectAcceptedResult(await service.execute(publishDawnCommand({ commandId: commandId(`${prefix}-dawn-2`), expectedGameVersion: state!.gameVersion })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  expectAcceptedResult(await service.execute(openNominationsCommand({ commandId: commandId(`${prefix}-open-nominations-2`), expectedGameVersion: state!.gameVersion, payload: { commandType: "OpenNominations", dayNumber: state!.dayNumber } })));
  state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  if (state === undefined) throw new Error("Expected second nomination state");
  return { service, commandStore, state, deadPlayerId };
};

const expectSlice3ReceiptOnlyRejection = async (
  fixture: Slice3DayFixture,
  command: SupportedCommandEnvelope
): Promise<CommandRejected> => {
  const beforeEvents = await fixture.commandStore.loadDomainEvents(ids.game);
  const beforeState = rebuildOptionalGameState(beforeEvents);
  const result = await fixture.service.execute(command);
  expect(result).toMatchObject({ status: "rejected" });
  expect(await fixture.commandStore.loadDomainEvents(ids.game)).toStrictEqual(beforeEvents);
  expect(rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))).toStrictEqual(beforeState);
  expect(await fixture.commandStore.findCommandReceipt(ids.game, command.commandId)).toMatchObject({
    result: { status: "rejected" }
  });
  if (result.status !== "rejected") throw new Error("Expected a rejected Slice3 command");
  return result;
};

describeApplicationServiceShard("core", "Slice 3 command validation surface", () => {
  it("[S3-C01] accepts living nomination and dead nominee through the real command boundary", async () => {
    const fixture = await reachSlice3SecondDay("s3-c01");
    const deadIds = new Set([...(fixture.state.deadPlayerIds ?? []), ...(fixture.state.deaths ?? []).map((death) => death.playerId)]);
    const nominator = fixture.state.roster!.entries.find((entry) => !deadIds.has(entry.playerId))!;
    const result = await fixture.service.execute(declareNominationCommand({
      commandId: commandId("s3-c01-dead-nominee"), expectedGameVersion: fixture.state.gameVersion,
      actor: { kind: "ai", playerId: nominator.playerId }, payload: { commandType: "DeclareNomination", targetPlayerId: fixture.deadPlayerId }
    }));
    expectAcceptedResult(result);
    expect(result.events[0]).toMatchObject({ eventType: "NominationDeclared", payload: { nomineePlayerId: fixture.deadPlayerId } });
    const afterAccepted = await fixture.commandStore.loadDomainEvents(ids.game);
    expect(await fixture.commandStore.findCommandReceipt(ids.game, commandId("s3-c01-dead-nominee"))).toMatchObject({ result: { status: "accepted" } });
    const replay = await fixture.service.execute(declareNominationCommand({
      commandId: commandId("s3-c01-dead-nominee"), expectedGameVersion: fixture.state.gameVersion,
      actor: { kind: "ai", playerId: nominator.playerId }, payload: { commandType: "DeclareNomination", targetPlayerId: fixture.deadPlayerId }
    }));
    expectAcceptedResult(replay);
    expect(await fixture.commandStore.loadDomainEvents(ids.game)).toStrictEqual(afterAccepted);
    const alternateTarget = fixture.state.roster!.entries.find((entry) => !deadIds.has(entry.playerId) && entry.playerId !== nominator.playerId)!.playerId;
    await expectSlice3ReceiptOnlyRejection(fixture, declareNominationCommand({
      commandId: commandId("s3-c01-nominator-once-per-day"), expectedGameVersion: rebuildOptionalGameState(afterAccepted)!.gameVersion,
      actor: { kind: "ai", playerId: nominator.playerId }, payload: { commandType: "DeclareNomination", targetPlayerId: alternateTarget }
    }));
  }, 30_000);

  it("[S3-C02] rejects dead nominator and illegal nomination boundaries with receipt-only audit", async () => {
    const fixture = await reachSlice3SecondDay("s3-c02");
    const before = await fixture.commandStore.loadDomainEvents(ids.game);
    const beforeState = rebuildOptionalGameState(before);
    const deadIds = new Set([...(fixture.state.deadPlayerIds ?? []), ...(fixture.state.deaths ?? []).map((death) => death.playerId)]);
    const result = await fixture.service.execute(declareNominationCommand({
      commandId: commandId("s3-c02-dead-nominator"), expectedGameVersion: fixture.state.gameVersion,
      actor: { kind: "ai", playerId: fixture.deadPlayerId },
      payload: { commandType: "DeclareNomination", targetPlayerId: fixture.state.roster!.entries.find((entry) => !deadIds.has(entry.playerId) && entry.playerId !== fixture.deadPlayerId)!.playerId }
    }));
    expect(result).toMatchObject({ status: "rejected" });
    expect(await fixture.commandStore.loadDomainEvents(ids.game)).toStrictEqual(before);
    expect(rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))).toStrictEqual(beforeState);
    expect(await fixture.commandStore.findCommandReceipt(ids.game, commandId("s3-c02-dead-nominator"))).toMatchObject({ result: { status: "rejected" } });
    await expectSlice3ReceiptOnlyRejection(fixture, openNominationsCommand({
      commandId: commandId("s3-c02-wrong-day"), expectedGameVersion: fixture.state.gameVersion,
      payload: { commandType: "OpenNominations", dayNumber: fixture.state.dayNumber + 1 }
    }));
  }, 30_000);

  it("[S3-C03] validates living vote, ghost first use, no-vote, and second-use rejection", async () => {
    const fixture = await reachSlice3SecondDay("s3-c03");
    const deadIds = new Set([...(fixture.state.deadPlayerIds ?? []), ...(fixture.state.deaths ?? []).map((death) => death.playerId)]);
    const nominator = fixture.state.roster!.entries.find((entry) => !deadIds.has(entry.playerId))!;
    const livingVoter = fixture.state.roster!.entries.find((entry) => !deadIds.has(entry.playerId) && entry.playerId !== nominator.playerId)!;
    const target = fixture.state.roster!.entries.find((entry) => !deadIds.has(entry.playerId) && entry.playerId !== nominator.playerId && entry.playerId !== livingVoter.playerId)!;
    expectAcceptedResult(await fixture.service.execute(declareNominationCommand({ commandId: commandId("s3-c03-nomination-a"), expectedGameVersion: fixture.state.gameVersion, actor: { kind: "ai", playerId: nominator.playerId }, payload: { commandType: "DeclareNomination", targetPlayerId: target.playerId } })));
    let state = rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))!;
    const nominationId = state.nominations!.at(-1)!.nominationId;
    expectAcceptedResult(await fixture.service.execute(openVoteCommand({ commandId: commandId("s3-c03-open-vote-a"), expectedGameVersion: state.gameVersion, payload: { commandType: "OpenVote", nominationId } })));
    state = rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))!;
    const livingYes = await fixture.service.execute(castVoteCommand({ commandId: commandId("s3-c03-living-yes"), expectedGameVersion: state.gameVersion, actor: { kind: "ai", playerId: livingVoter.playerId }, payload: { commandType: "CastVote", nominationId, choice: "YES" } }));
    expectAcceptedResult(livingYes);
    expect(livingYes.events[0]).toMatchObject({ eventType: "VoteCast", payload: { voterPlayerId: livingVoter.playerId, choice: "YES", ghostVoteConsumed: false } });
    state = rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))!;
    const noVote = await fixture.service.execute(castVoteCommand({ commandId: commandId("s3-c03-dead-no-a"), expectedGameVersion: state.gameVersion, actor: { kind: "ai", playerId: fixture.deadPlayerId }, payload: { commandType: "CastVote", nominationId, choice: "NO" } }));
    ...
    state = rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))!;
    expectAcceptedResult(await fixture.service.execute(completeVoteCommand({ commandId: commandId("s3-c03-complete-a"), expectedGameVersion: state.gameVersion, payload: { commandType: "CompleteVote", nominationId } })));
    state = rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))!;

    const nominatorB = fixture.state.roster!.entries.find((entry) => !deadIds.has(entry.playerId) && entry.playerId !== nominator.playerId)!;
    const targetB = fixture.state.roster!.entries.find((entry) => !deadIds.has(entry.playerId) && entry.playerId !== nominator.playerId && entry.playerId !== nominatorB.playerId && entry.playerId !== target.playerId)!;
    expectAcceptedResult(await fixture.service.execute(declareNominationCommand({ commandId: commandId("s3-c03-nomination-b"), expectedGameVersion: state.gameVersion, actor: { kind: "ai", playerId: nominatorB.playerId }, payload: { commandType: "DeclareNomination", targetPlayerId: targetB.playerId } })));
    state = rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))!;
    const nominationB = state.nominations!.at(-1)!.nominationId;
    expectAcceptedResult(await fixture.service.execute(openVoteCommand({ commandId: commandId("s3-c03-open-vote-b"), expectedGameVersion: state.gameVersion, payload: { commandType: "OpenVote", nominationId: nominationB } })));
    state = rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))!;
    const yesVote = await fixture.service.execute(castVoteCommand({ commandId: commandId("s3-c03-dead-yes-b"), expectedGameVersion: state.gameVersion, actor: { kind: "ai", playerId: fixture.deadPlayerId }, payload: { commandType: "CastVote", nominationId: nominationB, choice: "YES" } }));
    expectAcceptedResult(yesVote);
    expect(yesVote.events[0]).toMatchObject({ eventType: "VoteCast", payload: { choice: "YES", ghostVoteConsumed: true } });
    const beforeDuplicateUse = await fixture.commandStore.loadDomainEvents(ids.game);
    await expectSlice3ReceiptOnlyRejection(fixture, castVoteCommand({ commandId: commandId("s3-c03-dead-duplicate-b"), expectedGameVersion: rebuildOptionalGameState(beforeDuplicateUse)!.gameVersion, actor: { kind: "ai", playerId: fixture.deadPlayerId }, payload: { commandType: "CastVote", nominationId: nominationB, choice: "YES" } }));
    state = rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))!;
    expectAcceptedResult(await fixture.service.execute(completeVoteCommand({ commandId: commandId("s3-c03-complete-b"), expectedGameVersion: state.gameVersion, payload: { commandType: "CompleteVote", nominationId: nominationB } })));
    state = rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))!;

    const nominatorC = fixture.state.roster!.entries.find((entry) => !deadIds.has(entry.playerId) && entry.playerId !== nominator.playerId && entry.playerId !== nominatorB.playerId)!;
    const nominationCTarget = fixture.state.roster!.entries.find((entry) => !deadIds.has(entry.playerId) && entry.playerId !== nominator.playerId && entry.playerId !== nominatorB.playerId && entry.playerId !== nominatorC.playerId && entry.playerId !== target.playerId && entry.playerId !== targetB.playerId)!;
    expectAcceptedResult(await fixture.service.execute(declareNominationCommand({ commandId: commandId("s3-c03-nomination-c"), expectedGameVersion: state.gameVersion, actor: { kind: "ai", playerId: nominatorC.playerId }, payload: { commandType: "DeclareNomination", targetPlayerId: nominationCTarget.playerId } })));
    state = rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))!;
    const nominationC = state.nominations!.at(-1)!.nominationId;
    expectAcceptedResult(await fixture.service.execute(openVoteCommand({ commandId: commandId("s3-c03-open-vote-c"), expectedGameVersion: state.gameVersion, payload: { commandType: "OpenVote", nominationId: nominationC } })));
    state = rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))!;
    const forgedGhostProvenance = (await fixture.commandStore.loadDomainEvents(ids.game)).map((event) => event.eventType === "VoteCast" && event.payload.voterPlayerId === fixture.deadPlayerId && event.payload.nominationId === nominationB
      ? { ...event, payload: { ...event.payload, ghostVoteConsumed: false } }
      : event);
    expect(() => rebuildOptionalGameState(forgedGhostProvenance)).toThrow();
    await expectSlice3ReceiptOnlyRejection(fixture, castVoteCommand({
      commandId: commandId("s3-c03-dead-exhausted-c"), expectedGameVersion: state.gameVersion,
      actor: { kind: "ai", playerId: fixture.deadPlayerId }, payload: { commandType: "CastVote", nominationId: nominationC, choice: "YES" }
    }));
  }, 30_000);
  ...
  it("[S3-C04] rejects incomplete execution lifecycle and preserves execution/death provenance", async () => {
    ...
    const completedBlock = priorEvents.find((event) => event.eventType === "BlockStateUpdated");
    ...
    if (declared === undefined || completedBlock === undefined || resolved === undefined || death === undefined || resolved.eventType !== "ExecutionResolved" || declared.eventType !== "ExecutionDeclared" || completedBlock.eventType !== "BlockStateUpdated" || death.eventType !== "PlayerDied") {
      throw new Error("Expected the canonical execution/death chain");
    }
    expect(completedBlock.payload.leaderNominationId).not.toBeNull();
    expect(completedBlock.payload.leaderVoteCount).toBeGreaterThanOrEqual(completedBlock.payload.threshold);
    expect(completedBlock.payload.tied).toBe(false);
    const winningNomination = priorEvents.find((event): event is Extract<AnyDomainEventEnvelope, { readonly eventType: "NominationDeclared" }> =>
      event.eventType === "NominationDeclared" && event.payload.nominationId === completedBlock.payload.leaderNominationId);
    expect(declared.payload.targetPlayerId).toBe(winningNomination?.payload.nomineePlayerId);
    ...
    const before = await fixture.commandStore.loadDomainEvents(ids.game);
    await expectSlice3ReceiptOnlyRejection(fixture, closeNominationsCommand({
  ...
  it("[S3-C05] enforces the exact current command actor-phase-prerequisite matrix", async () => {
    const fixture = await reachSlice3SecondDay("s3-c05");
    const deadIds = new Set([...(fixture.state.deadPlayerIds ?? []), ...(fixture.state.deaths ?? []).map((death) => death.playerId)]);
    ...
    const matrixVersion = fixture.state.gameVersion;
    await expectSlice3ReceiptOnlyRejection(fixture, openNominationsCommand({
      commandId: commandId("s3-c05-reject-open-nominations"), expectedGameVersion: matrixVersion,
      payload: { commandType: "OpenNominations", dayNumber: fixture.state.dayNumber }
    }));
    await expectSlice3ReceiptOnlyRejection(fixture, declareNominationCommand({
      commandId: commandId("s3-c05-reject-declare-dead"), expectedGameVersion: matrixVersion,
      actor: { kind: "ai", playerId: fixture.deadPlayerId }, payload: { commandType: "DeclareNomination", targetPlayerId: liveTarget.playerId }
    }));
    await expectSlice3ReceiptOnlyRejection(fixture, openVoteCommand({
      commandId: commandId("s3-c05-reject-open-vote"), expectedGameVersion: matrixVersion,
      payload: { commandType: "OpenVote", nominationId: "nomination-v1:2:999" }
    }));
    await expectSlice3ReceiptOnlyRejection(fixture, castVoteCommand({
      commandId: commandId("s3-c05-reject-cast-vote"), expectedGameVersion: matrixVersion,
      actor: { kind: "ai", playerId: liveNominator.playerId }, payload: { commandType: "CastVote", nominationId: "nomination-v1:2:999", choice: "YES" }
    }));
    await expectSlice3ReceiptOnlyRejection(fixture, completeVoteCommand({
      commandId: commandId("s3-c05-reject-complete-vote"), expectedGameVersion: matrixVersion,
      payload: { commandType: "CompleteVote", nominationId: "nomination-v1:2:999" }
    }));
    await expectSlice3ReceiptOnlyRejection(fixture, closeNominationsCommand({
      commandId: commandId("s3-c05-reject-close-nominations"), expectedGameVersion: matrixVersion,
      payload: { commandType: "CloseNominations", dayNumber: fixture.state.dayNumber }
    }));
    await expectSlice3ReceiptOnlyRejection(fixture, resolveExecutionCommand({
      commandId: commandId("s3-c05-reject-resolve-execution"), expectedGameVersion: matrixVersion,
      payload: { commandType: "ResolveExecution", blockId: "block-v1:2:2" }
    }));
    await expectSlice3ReceiptOnlyRejection(fixture, completeNightCommand({
      commandId: commandId("s3-c05-reject-complete-night"), expectedGameVersion: matrixVersion,
      payload: { commandType: "CompleteNight", phase: "FIRST_NIGHT", planVersion: "first-night-task-plan-v2", nightNumber: fixture.state.nightNumber }
    }));
    await expectSlice3ReceiptOnlyRejection(fixture, publishDawnCommand({
      commandId: commandId("s3-c05-reject-publish-dawn"), expectedGameVersion: matrixVersion
    }));
    await expectSlice3ReceiptOnlyRejection(fixture, beginNightCommand({
      commandId: commandId("s3-c05-reject-begin-night"), expectedGameVersion: matrixVersion, actor: { kind: "human", playerId: fixture.deadPlayerId }
    }));
    await expectSlice3ReceiptOnlyRejection(fixture, settleOrdinaryNightTaskCommand({
      commandId: commandId("s3-c05-reject-settle-ordinary"), expectedGameVersion: matrixVersion,
      payload: { commandType: "SettleOrdinaryNightTask", taskId: scheduledTaskId("ordinary-night-v1:GENERIC_DEMON_KILL:night-02:seat-99") }
    }));
    ...
    matrixState = rebuildOptionalGameState(await fixture.commandStore.loadDomainEvents(ids.game))!;
    await expectSlice3ReceiptOnlyRejection(fixture, openVoteCommand({
      commandId: commandId("s3-c05-matrix-reopen-vote"), expectedGameVersion: matrixState.gameVersion,
      payload: { commandType: "OpenVote", nominationId }
    }));
  }, 30_000);
});
  ...
  for (let step = 0; step < 24; step += 1) {
    ...
    if (next.taskType === "SNAKE_CHARMER_ACTION" || next.taskType === "WITCH_ACTION" || next.taskType === "DREAMER_ACTION" || next.taskType === "SEAMSTRESS_ACTION") {
      ...
      const target = openState.currentCharacterState?.entries.find((entry) =>
        entry.playerId !== opportunity.sourcePlayerId && (next.taskType !== "SNAKE_CHARMER_ACTION" ||
          (chooseSnakeDemon ? entry.role.characterType === "DEMON" : entry.role.characterType !== "DEMON"))
      );
...
const reachOpenPhilosopherGainedDreamerOpportunity = async (
  service: GameApplicationService,
  commandStore: MemoryCommandCommitStore,
  idPrefix: string,
  chooseSnakeDemon = false
  ...
  const philosopher = await reachOpenExactPhilosopherOpportunity(
    service,
    commandStore,
    philosopherDreamerExactRoleIds
  );
  expectAcceptedResult(await service.execute(
    chooseExactPhilosopherRole("dreamer", philosopher, `${idPrefix}-choose-dreamer`)
  ));
  const afterChoice = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  const task = afterChoice?.firstNightTaskPlan?.tasks.find((entry) =>
    entry.taskType === "DREAMER_ACTION" && entry.source.kind === "PHILOSOPHER_GAINED_ABILITY"
  );
  if (afterChoice === undefined || task === undefined) throw new Error("Expected gained Dreamer task");
  const ready = await advanceToScheduledTask(
    service,
    commandStore,
    task.taskId,
  ...
  expectAcceptedResult(await service.execute(openFirstNightRoleActionOpportunityCommand({
    commandId: commandId(`${idPrefix}-open-gained-dreamer`),
    expectedGameVersion: ready.gameVersion,
    payload: { commandType: "OpenFirstNightRoleActionOpportunity", taskId: task.taskId }
  })));
  const state = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  const opportunity = state?.firstNightActionOpportunities?.opportunities.find((entry) =>
    entry.taskId === task.taskId && entry.opportunityKind === "DREAMER_FIRST_NIGHT_ACTION_V4"
  );
  if (state === undefined || opportunity === undefined) throw new Error("Expected opened gained Dreamer V4 opportunity");
  return { state, task, opportunity, afterChoice };
};

const settlePhilosopherGainedVortoxDreamer = async (
  service: GameApplicationService,
  commandStore: MemoryCommandCommitStore,
  idPrefix: string
) => {
  const native = await reachCanonicalDrunkVortoxDreamerOpportunity(service, commandStore, `${idPrefix}-native`);
  ...
  if (nativeTarget === undefined) throw new Error("Expected native Dreamer bridge target");
  expectAcceptedResult(await service.execute(submitDreamerActionCommand({
    commandId: commandId(`${idPrefix}-submit-native`), expectedGameVersion: native.state.gameVersion,
  ...
  const beforeGained = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  const task = beforeGained?.firstNightTaskPlan?.tasks.find((entry) =>
    entry.taskType === "DREAMER_ACTION" && entry.source.kind === "PHILOSOPHER_GAINED_ABILITY"
  );
  if (beforeGained === undefined || task === undefined) throw new Error("Expected gained Vortox Dreamer task");
  expectAcceptedResult(await service.execute(openFirstNightRoleActionOpportunityCommand({
    commandId: commandId(`${idPrefix}-open-gained`), expectedGameVersion: beforeGained.gameVersion,
    payload: { commandType: "OpenFirstNightRoleActionOpportunity", taskId: task.taskId }
  })));
  const opened = rebuildOptionalGameState(await commandStore.loadDomainEvents(ids.game));
  ...
  const target = opened?.currentCharacterState?.entries.find((entry) =>
    entry.playerId !== opportunity?.sourcePlayerId && entry.role.roleId !== "vortox"
  );
  if (opened === undefined || opportunity === undefined || target === undefined) throw new Error("Expected opened gained Vortox Dreamer");
  const beforeSubmitCount = (await commandStore.loadDomainEvents(ids.game)).length;
  expectAcceptedResult(await service.execute(submitDreamerActionCommand({
    commandId: commandId(`${idPrefix}-submit-gained`), expectedGameVersion: opened.gameVersion,
  ...
  if (state === undefined) throw new Error("Expected rebuilt gained Vortox Dreamer state");
...
describeApplicationServiceShard("dreamer-vortox", "Phase 3 Slice 2B19A3B1 canonical-drunk Vortox Dreamer", () => {
  it("[2B19B-C01/C09/C10/C42] opens one exact gained Dreamer V4 opportunity and preserves base Snake Charmer", async () => {
    ...
    const sourceTenures = openedState.seamstressRoleTenureState?.records.filter((entry) =>
      entry.playerId === opened.sourcePlayerId
    ) ?? [];
    expect(sourceTenures.some((entry) => entry.roleId === "philosopher")).toBe(true);
    expect(sourceTenures.some((entry) => entry.roleId === "dreamer")).toBe(false);
    ...
    expect(normalState?.firstNightActionOpportunities?.opportunities.find((entry) =>
      entry.opportunityId === opened.opportunityId)).toMatchObject({
      opportunityKind: "DREAMER_FIRST_NIGHT_ACTION_V4",
      opportunityStatus: "CLOSED",
      opportunityId: opened.opportunityId
    });
  ...
  it("[2B19B-C20/C21/C22/C24/C25/C26/C30/C31/C32/C58] proves accepted V5 and V6 GOOD/EVIL settlement authority", async () => {
    for (const alignment of ["GOOD", "EVIL"] as const) {
      const store = new MemoryCommandCommitStore();
      const { service } = makeService(store);
      const opened = await reachOpenPhilosopherGainedDreamerOpportunity(
        service,
        store,
        `2b19b-normal-${alignment.toLowerCase()}`,
        alignment === "GOOD"
      );
      const target = opened.state.currentCharacterState?.entries.find((entry) => {
        const prior = opened.afterChoice.currentCharacterState?.entries.find((candidate) => candidate.playerId === entry.playerId);
        return entry.playerId !== opened.opportunity.sourcePlayerId && entry.role.defaultAlignment === alignment &&
          (alignment !== "GOOD" || prior?.role.roleId !== entry.role.roleId);
      });
      if (target === undefined) throw new Error(`Expected ${alignment} gained Dreamer target`);
      const original = opened.afterChoice.currentCharacterState?.entries.find((entry) => entry.playerId === target.playerId);
      expectAcceptedResult(await service.execute(submitDreamerActionCommand({
        commandId: commandId(`2b19b-normal-${alignment.toLowerCase()}-submit`),
        expectedGameVersion: opened.state.gameVersion,
        payload: { commandType: "SubmitDreamerAction", taskId: opened.task.taskId,
          opportunityId: opened.opportunity.opportunityId,
          decision: { kind: "CHOOSE_PLAYER", targetPlayerId: target.playerId } }
      })));
      const events = await store.loadDomainEvents(ids.game);
      const rebuilt = rebuildOptionalGameState(events);
      const delivery = rebuilt?.dreamerInformation?.deliveries.find((entry) => entry.taskId === opened.task.taskId);
      expect(delivery).toMatchObject({
        deliverySchemaVersion: "dreamer-information-delivered-v5",
        informationReliability: { kind: "PHILOSOPHER_GAINED_EFFECTIVE" },
        evaluatedCharacterStateRevision: opened.state.currentCharacterState?.revision
      });
      expect(alignment === "GOOD" ? delivery?.goodRole.roleId : delivery?.evilRole.roleId).toBe(target.role.roleId);
      const seamstressTask = rebuilt?.firstNightTaskPlan?.tasks.find((candidate) =>
        !rebuilt.firstNightTaskProgress?.settlements.some((settlement) => settlement.taskId === candidate.taskId)
      );
      if (rebuilt === undefined || seamstressTask?.taskType !== "SEAMSTRESS_ACTION") {
        throw new Error("Expected terminal Seamstress boundary after gained Dreamer");
      }
      expectAcceptedResult(await service.execute(openFirstNightRoleActionOpportunityCommand({
        commandId: commandId(`2b19b-normal-${alignment.toLowerCase()}-open-seamstress`),
        expectedGameVersion: rebuilt.gameVersion,
      ...
      if (seamstressOpen === undefined || seamstressOpportunity === undefined) throw new Error("Expected opened Seamstress boundary");
      expectEventSummaryAcceptedResult(await service.execute(submitSeamstressActionCommand({
        commandId: commandId(`2b19b-normal-${alignment.toLowerCase()}-defer-seamstress`),
        expectedGameVersion: seamstressOpen.gameVersion,
      ...
      const postSeamstressEvents = await store.loadDomainEvents(ids.game);
      const postSeamstress = rebuildOptionalGameState(postSeamstressEvents);
      const nextUnsettled = postSeamstress?.firstNightTaskPlan?.tasks.find((candidate) =>
        !postSeamstress.firstNightTaskProgress?.settlements.some((settlement) => settlement.taskId === candidate.taskId)
      );
      expect(nextUnsettled?.taskType).toBe("MATHEMATICIAN_INFORMATION");
      expect(postSeamstress?.mathematicianInformation?.deliveries ?? []).toHaveLength(0);
      expect(postSeamstressEvents.some((event) => event.eventType === "MathematicianInformationDelivered")).toBe(false);
      if (alignment === "GOOD") {
        expect(opened.state.currentCharacterState?.revision).toBeGreaterThan(opened.afterChoice.currentCharacterState?.revision ?? 0);
        expect(original?.role.roleId).not.toBe(target.role.roleId);
      }
      expect(rebuildOptionalGameState(structuredClone(events))).toStrictEqual(rebuilt);
    }

    for (const alignment of ["GOOD", "EVIL"] as const) {
      const store = new MemoryCommandCommitStore();
      const { service } = makeService(store);
      const native = await reachCanonicalDrunkVortoxDreamerOpportunity(service, store, `2b19b-vortox-${alignment.toLowerCase()}`);
      const nativeTarget = native.state.currentCharacterState?.entries.find((entry) =>
        entry.playerId !== native.opportunity.sourcePlayerId && entry.role.roleId !== "vortox"
      );
      if (nativeTarget === undefined) throw new Error("Expected native Dreamer target");
      expectAcceptedResult(await service.execute(submitDreamerActionCommand({
        commandId: commandId(`2b19b-vortox-${alignment.toLowerCase()}-native`), expectedGameVersion: native.state.gameVersion,
        payload: { commandType: "SubmitDreamerAction", taskId: native.dreamerTask.taskId,
          opportunityId: native.opportunity.opportunityId,
          decision: { kind: "CHOOSE_PLAYER", targetPlayerId: nativeTarget.playerId } }
      })));
      ...
      if (beforeGained === undefined || task === undefined) throw new Error("Expected Vortox gained Dreamer task");
      expectAcceptedResult(await service.execute(openFirstNightRoleActionOpportunityCommand({
        commandId: commandId(`2b19b-vortox-${alignment.toLowerCase()}-open-gained`), expectedGameVersion: beforeGained.gameVersion,
        payload: { commandType: "OpenFirstNightRoleActionOpportunity", taskId: task.taskId }
      })));
      const opened = rebuildOptionalGameState(await store.loadDomainEvents(ids.game));
      const opportunity = opened?.firstNightActionOpportunities?.opportunities.find((entry) => entry.taskId === task.taskId);
      const target = opened?.currentCharacterState?.entries.find((entry) =>
        entry.playerId !== opportunity?.sourcePlayerId && entry.role.roleId !== "vortox" && entry.role.defaultAlignment === alignment
      );
      if (opened === undefined || opportunity === undefined || target === undefined) throw new Error(`Expected Vortox ${alignment} target`);
      const nativeImpairment = opened.abilityImpairments?.impairments.find((entry) =>
        entry.kind === "DRUNK" && entry.affectedPlayerId === native.opportunity.sourcePlayerId
      );
      expect(nativeImpairment).toBeDefined();
      expect(opportunity.sourcePlayerId).not.toBe(native.opportunity.sourcePlayerId);
      expectAcceptedResult(await service.execute(submitDreamerActionCommand({
        commandId: commandId(`2b19b-vortox-${alignment.toLowerCase()}-submit-gained`), expectedGameVersion: opened.gameVersion,
        payload: { commandType: "SubmitDreamerAction", taskId: task.taskId, opportunityId: opportunity.opportunityId,
          decision: { kind: "CHOOSE_PLAYER", targetPlayerId: target.playerId } }
      })));
      const rebuilt = rebuildOptionalGameState(await store.loadDomainEvents(ids.game));
      const delivery = rebuilt?.dreamerInformation?.deliveries.find((entry) => entry.taskId === task.taskId);
      expect(delivery).toMatchObject({
        deliverySchemaVersion: "dreamer-information-delivered-v6",
        informationReliability: { kind: "VORTOX_FORCED_FALSE" }
      });
      expect([delivery?.goodRole.roleId, delivery?.evilRole.roleId]).not.toContain(target.role.roleId);
    }
  }, 60_000);
  ...
  it("[2B19B-C54] preserves accepted V1 gained Dreamer history without migration", async () => {
    const store = new MemoryCommandCommitStore();
    const { service } = makeService(store);
    const context = await reachOpenExactPhilosopherOpportunity(service, store);
    const acceptedV1History = await convertStoredFirstNightPlanToAcceptedV1(store);
    const before = rebuildOptionalGameState(acceptedV1History);
    if (before === undefined) throw new Error("Expected accepted V1 compatibility state");
    const command = chooseExactPhilosopherRole("dreamer", { ...context, state: before }, "legacy-v1-no-migration");
    await expect(service.execute(command)).resolves.toMatchObject({
      status: "failed", code: "ApplicationNotConfigured", failureStage: "first-night-role-action", retryable: true
    });
    const afterEvents = await store.loadDomainEvents(ids.game);
    expect(afterEvents).toStrictEqual(acceptedV1History);
    expect(rebuildOptionalGameState(afterEvents)).toStrictEqual(before);
    expect(afterEvents.some((event) => event.eventType === "FirstNightTaskInsertedV2")).toBe(false);
  ...
  it("[2B19B-C17/C18/C51/C53] keeps gained-command validation, rebuild, and receipt faults atomic and retryable", async () => {
    const prepare = async (store: MemoryCommandCommitStore, prefix: string) => {
      const { service } = makeService(store);
      const opened = await reachOpenPhilosopherGainedDreamerOpportunity(service, store, prefix);
      const target = opened.state.currentCharacterState?.entries.find((entry) =>
        entry.playerId !== opened.opportunity.sourcePlayerId
      );
      if (target === undefined) throw new Error("Expected gained Dreamer fault target");
      const command = submitDreamerActionCommand({
        commandId: commandId(`${prefix}-submit`), expectedGameVersion: opened.state.gameVersion,
        payload: { commandType: "SubmitDreamerAction", taskId: opened.task.taskId,
          opportunityId: opened.opportunity.opportunityId,
          decision: { kind: "CHOOSE_PLAYER", targetPlayerId: target.playerId } }
      });
      return { service, opened, command };
    };

    const rebuildStore = new MemoryCommandCommitStore();
    const rebuild = await prepare(rebuildStore, "2b19b-fault-rebuild");
    const rebuildEvents = await rebuildStore.loadDomainEvents(ids.game);
    const corrupted = rebuildEvents[1];
    if (corrupted === undefined) throw new Error("Expected accepted prefix event");
    const originalSequence = corrupted.eventSequence;
    (corrupted as { eventSequence: number }).eventSequence = originalSequence + 10;
    await expect(rebuild.service.execute(rebuild.command)).resolves.toMatchObject({
      status: "failed", code: "CanonicalStateRebuildFailed", failureStage: "state-rebuild", retryable: true
    });
    expect(await rebuildStore.findCommandReceipt(ids.game, rebuild.command.commandId)).toBeUndefined();
    (corrupted as { eventSequence: number }).eventSequence = originalSequence;
    expectAcceptedResult(await rebuild.service.execute(rebuild.command));
    await expect(rebuild.service.execute(rebuild.command)).resolves.toMatchObject({ status: "accepted", idempotent: true });

    const acceptedReceiptStore = new OneShotAcceptedReceiptPersistenceFailureStore();
    const acceptedReceipt = await prepare(acceptedReceiptStore, "2b19b-fault-accepted-receipt");
    const beforeAcceptedEvents = await acceptedReceiptStore.loadDomainEvents(ids.game);
    acceptedReceiptStore.failAcceptedReceiptPersistence = true;
    await expect(acceptedReceipt.service.execute(acceptedReceipt.command)).resolves.toMatchObject({
      status: "failed", code: "EventStoreAppendFailed", failureStage: "accepted-commit", retryable: true
    });
    expect(await acceptedReceiptStore.loadDomainEvents(ids.game)).toStrictEqual(beforeAcceptedEvents);
    expect(await acceptedReceiptStore.findCommandReceipt(ids.game, acceptedReceipt.command.commandId)).toBeUndefined();
    expectAcceptedResult(await acceptedReceipt.service.execute(acceptedReceipt.command));
    await expect(acceptedReceipt.service.execute(acceptedReceipt.command)).resolves.toMatchObject({ status: "accepted", idempotent: true });

    const rejectedReceiptStore = new MemoryCommandCommitStore();
    const rejectedReceipt = await prepare(rejectedReceiptStore, "2b19b-fault-rejected-receipt");
    const invalid = submitDreamerActionCommand({
      ...rejectedReceipt.command,
      commandId: commandId("2b19b-fault-rejected-receipt-invalid"),
      payload: { ...rejectedReceipt.command.payload,
        decision: { kind: "CHOOSE_PLAYER", targetPlayerId: rejectedReceipt.opened.opportunity.sourcePlayerId } }
    });
    const beforeRejectedEvents = await rejectedReceiptStore.loadDomainEvents(ids.game);
    rejectedReceiptStore.failRejectedReceiptWrite = true;
    await expect(rejectedReceipt.service.execute(invalid)).resolves.toMatchObject({
      status: "failed", code: "CommandReceiptWriteFailed", failureStage: "rejected-receipt-write", retryable: true
    });
    expect(await rejectedReceiptStore.loadDomainEvents(ids.game)).toStrictEqual(beforeRejectedEvents);
    expect(await rejectedReceiptStore.findCommandReceipt(ids.game, invalid.commandId)).toBeUndefined();
    await expect(rejectedReceipt.service.execute(invalid)).resolves.toMatchObject({
      status: "rejected", code: "InvalidDreamerTarget", idempotent: false
    });
    await expect(rejectedReceipt.service.execute(invalid)).resolves.toMatchObject({
      status: "rejected", code: "InvalidDreamerTarget", idempotent: true
    });
    const missing = submitDreamerActionCommand({
      ...rejectedReceipt.command,
      commandId: commandId("2b19b-fault-missing-target"),
      payload: { ...rejectedReceipt.command.payload,
        decision: { kind: "CHOOSE_PLAYER", targetPlayerId: playerId("missing-dreamer-target") } }
    });
    await expect(rejectedReceipt.service.execute(missing)).resolves.toMatchObject({
      status: "rejected", code: "InvalidDreamerTarget", idempotent: false
    });
    expect(await rejectedReceiptStore.loadDomainEvents(ids.game)).toStrictEqual(beforeRejectedEvents);
  }, 60_000);

  it("[2B19B-C52] accepts the same gained command exactly once after a one-shot real-port fault", async () => {
    const store = new OneShotAcceptedReceiptPersistenceFailureStore();
    const { service } = makeService(store);
    const opened = await reachOpenPhilosopherGainedDreamerOpportunity(service, store, "2b19b-recovery");
    const target = opened.state.currentCharacterState?.entries.find((entry) =>
      entry.playerId !== opened.opportunity.sourcePlayerId
    );
    if (target === undefined) throw new Error("Expected gained Dreamer recovery target");
    const command = submitDreamerActionCommand({
      commandId: commandId("2b19b-recovery-submit"), expectedGameVersion: opened.state.gameVersion,
      payload: { commandType: "SubmitDreamerAction", taskId: opened.task.taskId,
        opportunityId: opened.opportunity.opportunityId,
        decision: { kind: "CHOOSE_PLAYER", targetPlayerId: target.playerId } }
    });
    const before = await store.loadDomainEvents(ids.game);
    store.failAcceptedReceiptPersistence = true;
    await expect(service.execute(command)).resolves.toMatchObject({
      status: "failed", code: "EventStoreAppendFailed", failureStage: "accepted-commit", retryable: true
    });
    expect(await store.loadDomainEvents(ids.game)).toStrictEqual(before);
    expectAcceptedResult(await service.execute(command));
    const after = await store.loadDomainEvents(ids.game);
    expect(after.slice(before.length).map((event) => event.eventType)).toStrictEqual([
    ...
    expect(await store.loadDomainEvents(ids.game)).toStrictEqual(after);
  }, 30_000);
  ...
  it("[2B19B-C13/C14/C15/C16/C23/C35/C38/C39/C41-S17] rejects hostile accepted gained prefixes and batches", async () => {
    ...
    if (targetEvent?.eventType !== "DreamerTargetChosen" || deliveryEvent?.eventType !== "DreamerInformationDelivered" ||
        settlementEvent?.eventType !== "ScheduledTaskSettled") throw new Error("Expected exact hostile authority batch");
    ...
    for (const [name, mutate] of [
      ...
      ["insertion", (payload: Record<string, unknown>) => {
        ((payload.sourceContract as Record<string, unknown>).taskInsertionReference as Record<string, unknown>)
          .sourceCharacterStateRevision = 99;
      }],
      ["revision", (payload: Record<string, unknown>) => { payload.evaluatedCharacterStateRevision = 99; }],
      ["vortox", (payload: Record<string, unknown>) => {
        (payload.vortoxConstraint as Record<string, unknown>).vortoxPlayerId = "forged-vortox";
      }],
      ["truth", (payload: Record<string, unknown>) => { payload.goodRole = structuredClone(payload.evilRole); }]
    ] as const) {
      const stream = structuredClone(captured.events);
      const delivery = stream[captured.beforeSubmitCount + 1];
      if (delivery?.eventType !== "DreamerInformationDelivered") throw new Error("Expected hostile gained delivery");
      mutate(delivery.payload);
      expect(() => rebuildOptionalGameState(stream), name).toThrow(DomainError);
    }

    const tenureMutated = structuredClone(captured.events);
    const assigned = tenureMutated.find((event) => event.eventType === "CharactersAssigned");
    if (assigned === undefined || assigned.eventType !== "CharactersAssigned") throw new Error("Expected accepted assignment prefix");
    const sourceIndex = assigned.payload.assignments.findIndex((entry) => entry.playerId === captured.opportunity.sourcePlayerId);
    const otherIndex = assigned.payload.assignments.findIndex((entry, index) => index !== sourceIndex && entry.role.roleId !== "philosopher");
    if (sourceIndex < 0 || otherIndex < 0) throw new Error("Expected tenure mutation assignments");
    const mutable = assigned.payload.assignments as unknown as Array<(typeof assigned.payload.assignments)[number]>;
    const sourceRole = mutable[sourceIndex]!.role;
    mutable[sourceIndex] = { ...mutable[sourceIndex]!, role: mutable[otherIndex]!.role };
    mutable[otherIndex] = { ...mutable[otherIndex]!, role: sourceRole };
    expect(() => rebuildOptionalGameState(tenureMutated)).toThrow(DomainError);
    ...
    for (const batch of [
      ...
      [targetEvent, { ...deliveryEvent, batchId: batchId("2b19b-hostile-split") }, settlementEvent],
      [targetEvent, { ...deliveryEvent, commandId: commandId("2b19b-hostile-cross-command") }, settlementEvent]
    ] as const) {
      expect(() => validateDomainBatchSemantics(captured.opened, batch)).toThrow(DomainError);
    }
  }, 30_000);
  ...
  it("[2B19B-C46/C47/C48/C49/C50-S19] projects gained information only from accepted history", async () => {
    ...
    expect(playerView.dreamerInformation).toStrictEqual({
      target: { playerId: captured.target.playerId, seatNumber: captured.target.seatNumber },
      goodRole: delivery?.goodRole,
      evilRole: delivery?.evilRole
    });
    expect(aiView.dreamerInformation).toStrictEqual(playerView.dreamerInformation);
    for (const entry of captured.state.currentCharacterState?.entries ?? []) {
      if (entry.playerId !== captured.opportunity.sourcePlayerId &&
          !captured.state.dreamerInformation?.deliveries.some((item) => item.sourcePlayerId === entry.playerId)) {
        expect(buildPlayerPrivateKnowledgeViewFromAcceptedEventStream(captured.events, entry.playerId).dreamerInformation).toBeUndefined();
      }
    }
    expect(Object.keys(playerView.dreamerInformation ?? {}).sort()).toStrictEqual(["evilRole", "goodRole", "target"]);
    if (playerView.dreamerInformation !== undefined) {
      (playerView.dreamerInformation.goodRole as { roleId: string }).roleId = "caller-mutation";
    }
    expect(buildPlayerPrivateKnowledgeViewFromAcceptedEventStream(captured.events, captured.opportunity.sourcePlayerId)
      .dreamerInformation?.goodRole).toStrictEqual(delivery?.goodRole);
    expect(() => buildPlayerPrivateKnowledgeView(captured.state, captured.opportunity.sourcePlayerId)).toThrow("accepted-event-stream");
  }, 30_000);
  ...
  it("[2B19B-S18] rejects missing and duplicate gained Dreamer evidence references", async () => {
    ...
    for (let index = 0; index < fact.evidenceReferences.length; index += 1) {
      expect(validateFirstNightAbilityOutcomeFactShape({
        ...fact,
        evidenceReferences: fact.evidenceReferences.filter((_entry, entryIndex) => entryIndex !== index)
      }).valid).toBe(false);
    }
    expect(validateFirstNightAbilityOutcomeFactShape({
      ...fact,
      evidenceReferences: [...fact.evidenceReferences, fact.evidenceReferences[0]!]
    }).valid).toBe(false);
  }, 30_000);
  ...
  it("[2B19B-C02/C03/C07/C37/C40/C43/C44/C45/C57] executes gained Dreamer V6 from the accepted bridge", async () => {
    const { service, commandStore } = makeService();
    const opened = await reachCanonicalDrunkVortoxDreamerOpportunity(
      service, commandStore, "2b19a3b1-boundary"
    );
    ...
    const tenureMutated = structuredClone(accepted);
    const assigned = tenureMutated.find((event) => event.eventType === "CharactersAssigned");
    if (assigned === undefined || assigned.eventType !== "CharactersAssigned") throw new Error("Expected accepted assignment prefix");
    const sourceAssignmentIndex = assigned.payload.assignments.findIndex((entry) =>
      entry.playerId === gainedOpportunity.sourcePlayerId
    );
    const otherAssignmentIndex = assigned.payload.assignments.findIndex((entry, index) =>
      index !== sourceAssignmentIndex && entry.role.roleId !== "philosopher"
    );
    if (sourceAssignmentIndex < 0 || otherAssignmentIndex < 0) throw new Error("Expected tenure mutation pair");
    const sourceRole = assigned.payload.assignments[sourceAssignmentIndex]!.role;
    const otherRole = assigned.payload.assignments[otherAssignmentIndex]!.role;
    (assigned.payload.assignments as unknown as Array<{ role: typeof sourceRole }>)[sourceAssignmentIndex] = {
      ...assigned.payload.assignments[sourceAssignmentIndex]!, role: otherRole
    };
    (assigned.payload.assignments as unknown as Array<{ role: typeof sourceRole }>)[otherAssignmentIndex] = {
      ...assigned.payload.assignments[otherAssignmentIndex]!, role: sourceRole
    };
    expect(() => rebuildOptionalGameState(tenureMutated)).toThrow(DomainError);

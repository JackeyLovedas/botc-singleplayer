...
const parseTask = (value: unknown): { readonly sourceKind: "BASE" | "PHILOSOPHER_GAINED"; readonly seatNumber: SeatNumber } | undefined => {
  ...
  const gained = /^first-night-v2:PHILOSOPHER_GAINED:DREAMER_ACTION:seat-(0[1-9]|1[0-2]):from-dreamer$/.exec(value);
  return gained?.[1] === undefined ? undefined : { sourceKind: "PHILOSOPHER_GAINED", seatNumber: Number(gained[1]) as SeatNumber };
};
...
const entitlementKeys = [
  "entitlementVersion", "taskPlanVersion", "schedulingVersion", "insertedTaskId", "insertedTaskType",
  "philosopherTaskId", "philosopherOpportunityId", "grantId", "sourcePlayerId", "sourceSeatNumber",
  "sourceCharacterStateRevision", "sourceRole", "chosenRole", "originalOpportunityKind", "originalOpportunityStatus",
  "insertionEventType"
] as const;
const validateEntitlement = (value: unknown, contract: DreamerV2SourceContract): value is DreamerV2GainedEntitlement => {
  if (!exact(value, entitlementKeys) || value.entitlementVersion !== "dreamer-philosopher-gained-entitlement-v2" ||
      value.taskPlanVersion !== "first-night-task-plan-v2" || value.schedulingVersion !== "philosopher-gained-first-night-scheduling-v2" ||
      value.insertedTaskId !== contract.taskId || value.insertedTaskType !== "DREAMER_ACTION" ||
      value.sourcePlayerId !== contract.sourcePlayerId || value.sourceSeatNumber !== contract.sourceSeatNumber ||
      !revision(value.sourceCharacterStateRevision) || !exactRole(value.sourceRole) || !exactRole(value.chosenRole) ||
      value.sourceRole.roleId !== "philosopher" || value.chosenRole.roleId !== "dreamer" ||
      !sameCanonicalDataValue(value.sourceRole, contract.sourceRole) || !sameCanonicalDataValue(value.chosenRole, contract.abilityRole) ||
      value.originalOpportunityKind !== "PHILOSOPHER_FIRST_NIGHT_ACTION" || value.originalOpportunityStatus !== "CLOSED" ||
      value.insertionEventType !== "FirstNightTaskInsertedV2") return false;
  const seatToken = String(contract.sourceSeatNumber).padStart(2, "0");
  return value.philosopherTaskId === `first-night-v1:PHILOSOPHER_ACTION:seat-${seatToken}` &&
    value.philosopherOpportunityId === `first-night-v1:PHILOSOPHER_ACTION:seat-${seatToken}:opportunity-01` &&
    value.grantId === `philosopher-grant-v1:seat-${seatToken}:from-dreamer`;
};
...
const gainedKeys = [...baseKeys, "schedulingVersion", "gainedEntitlement"] as const;

export const validateDreamerV2SourceContractShapeForInternalUse = (
  value: unknown
): DreamerV2InternalShapeValidation => {
  try {
    if (!exact(value, baseKeys) && !exact(value, gainedKeys)) return invalid("Dreamer V2 source contract must have one exact variant shape");
    if (value.kind !== "BASE_DREAMER_V2" && value.kind !== "PHILOSOPHER_GAINED_DREAMER_V2") return invalid("Dreamer V2 source kind is invalid");
    ...
    if (contract.kind === "BASE_DREAMER_V2") {
      return exact(value, baseKeys) && task.sourceKind === "BASE" && contract.sourceRole.roleId === "dreamer" &&
    ...
    if (!exact(value, gainedKeys) || task.sourceKind !== "PHILOSOPHER_GAINED" || contract.sourceRole.roleId !== "philosopher" ||
        contract.schedulingVersion !== "philosopher-gained-first-night-scheduling-v2" ||
        contract.abilityInstance.kind !== "PHILOSOPHER_GAINED_TASK_V2" || !validateEntitlement(contract.gainedEntitlement, contract) ||
        contract.abilityInstance.philosopherOpportunityId !== contract.gainedEntitlement.philosopherOpportunityId ||
        contract.abilityInstance.grantId !== contract.gainedEntitlement.grantId ||
        contract.abilityInstance.sourceCharacterStateRevision !== contract.gainedEntitlement.sourceCharacterStateRevision ||
        contract.abilityInstance.schedulingVersion !== contract.schedulingVersion) {
      return invalid("Philosopher-gained Dreamer V2 source contract cross-links are invalid");
    }
    return { valid: true };
  } catch {

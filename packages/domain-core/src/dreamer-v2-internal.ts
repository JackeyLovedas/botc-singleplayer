...
import { formatBaseDreamerV2ActionOpportunityId, formatPhilosopherGainedDreamerV2ActionOpportunityId, validateFirstNightActionOpportunityStateShape } from "./first-night-action-opportunity.js";
import type {
...
const validateDreamerV2CrossLinks = (context: CanonicalDreamerV2ContextData): boolean => {
  ...
  for (const opportunity of v2Opportunities) {
    ...
    if (opportunity.sourceContract.kind === "BASE_DREAMER_V2") {
      ...
      if (task.source.kind !== "PHILOSOPHER_GAINED_ABILITY") return false;
      const gainedContract = opportunity.sourceContract;
      const insertionMatches = context.firstNightTaskInsertions.insertions.filter((entry) =>
        "grantId" in entry && entry.taskId === task.taskId && entry.grantId === gainedContract.gainedEntitlement.grantId
      );
      const insertion = insertionMatches[0] as FirstNightTaskInsertedV2Payload | undefined;
      const grantMatches = context.philosopherGrantedAbilities.abilities.filter((grant) =>
        grant.grantId === gainedContract.gainedEntitlement.grantId
      );
      const choiceMatches = context.philosopherAbilityChoices.choices.filter((choice) =>
        choice.opportunityId === gainedContract.gainedEntitlement.philosopherOpportunityId
      );
      const originalMatches = context.firstNightActionOpportunities.opportunities.filter((entry) =>
        entry.opportunityId === gainedContract.gainedEntitlement.philosopherOpportunityId
      );
      const grant = grantMatches[0]; const choice = choiceMatches[0]; const original = originalMatches[0];
      if (insertionMatches.length !== 1 || insertion === undefined || grantMatches.length !== 1 || grant === undefined ||
          choiceMatches.length !== 1 || choice === undefined || originalMatches.length !== 1 || original === undefined ||
          original.opportunityKind !== "PHILOSOPHER_FIRST_NIGHT_ACTION" || original.opportunityStatus !== "CLOSED" ||
          insertion.philosopherOpportunityId !== choice.opportunityId || insertion.grantId !== grant.grantId ||
          grant.grantedAtOpportunityId !== choice.opportunityId || grant.grantedAtTaskId !== choice.taskId ||
          insertion.sourcePlayerId !== choice.sourcePlayerId || insertion.sourceSeatNumber !== choice.sourceSeatNumber ||
          !sameRoleSetupSnapshot(insertion.sourceRole, choice.sourceRole) ||
          !sameRoleSetupSnapshot(insertion.chosenRole, choice.chosenRole) ||
          !sameCanonicalDataValue(gainedContract.gainedEntitlement, {
            ...
            philosopherTaskId: choice.taskId,
            ...
            sourceRole: insertion.sourceRole,
            chosenRole: insertion.chosenRole,
  ...
  for (const delivery of v2Deliveries) {
    ...
    if (delivery.sourceContract.kind === "PHILOSOPHER_GAINED_DREAMER_V2") {
      const grantEvidence = fact.evidenceReferences.filter((evidence) => evidence.kind === "PHILOSOPHER_GRANT");
      const insertionEvidence = fact.evidenceReferences.filter((evidence) => evidence.kind === "FIRST_NIGHT_TASK_INSERTION");
      if (grantEvidence.length !== 1 || grantEvidence[0]!.grantId !== delivery.sourceContract.gainedEntitlement.grantId ||
          insertionEvidence.length !== 1 || insertionEvidence[0]!.taskId !== delivery.taskId ||
          insertionEvidence[0]!.philosopherOpportunityId !== delivery.sourceContract.gainedEntitlement.philosopherOpportunityId ||
          insertionEvidence[0]!.generation.kind !== "V2" ||
          insertionEvidence[0]!.generation.grantId !== delivery.sourceContract.gainedEntitlement.grantId) return false;
    }
  }

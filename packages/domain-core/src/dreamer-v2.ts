...
export type PhilosopherGainedDreamerV2SourceContract = DreamerV2SourceBase & {
  readonly kind: "PHILOSOPHER_GAINED_DREAMER_V2";
  readonly schedulingVersion: "philosopher-gained-first-night-scheduling-v2";
  readonly gainedEntitlement: DreamerV2GainedEntitlement;
  readonly abilityInstance: Extract<FirstNightAbilityInstanceProvenance, { readonly kind: "PHILOSOPHER_GAINED_TASK_V2" }>;
};
...
export const orderDreamerV2GainedTasksForInternalValidation = (
  tasks: readonly ScheduledTask[]
): readonly ScheduledTask[] => [...tasks].sort((left, right) => {
  const leftSeat = left.source.kind === "PHILOSOPHER_GAINED_ABILITY" ? left.source.seatNumber : 0;
  const rightSeat = right.source.kind === "PHILOSOPHER_GAINED_ABILITY" ? right.source.seatNumber : 0;

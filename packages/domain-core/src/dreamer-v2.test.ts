...
import type { BaseDreamerV2SourceContract } from "./dreamer-v2.js";
import {
...
describe("Dreamer V2 pure and hostile domain seams", () => {
  it("D19A-001 PURE_POLICY_SEAM resolves the exact GOOD domain", () => {
  ...
  it("D19A-002 PURE_POLICY_SEAM resolves the exact EVIL domain", () => {
    const result = resolveDreamerV2Candidates({
  ...
  it("D19A-003 PURE_POLICY_SEAM uses UTF-16 code-unit ordering", () => {
    const result = resolveDreamerV2Candidates({
  ...
  it("D19A-004 PURE_POLICY_SEAM normal effective information preserves target truth", () => {
    const result = resolveDreamerV2Candidates({
  ...
  it("D19A-005 PURE_POLICY_SEAM Vortox information excludes target truth", () => {
    const result = resolveDreamerV2Candidates({
      roleCatalogSnapshot: catalog([dreamer, flowergirl, witch, vortox]), roleCatalogSignature: "signed",
  ...
  it("D19A-006 STRUCTURAL_VALIDATION rejects sparse catalogs", () => {
  ...
  it("D19A-007 STRUCTURAL_VALIDATION rejects duplicate candidates", () => {
    expect(resolveDreamerV2Candidates({
  ...
  it("D19A-008 STRUCTURAL_VALIDATION rejects an unknown target role", () => {
    expect(resolveDreamerV2Candidates({
  ...
  it("D19A-009 PURE_POLICY_SEAM is invariant to catalog input order", () => {
    const input = {
  ...
  it("D19A-010 PURE_POLICY_SEAM POISONED selects false when a legal alternative exists", () => {
    const poisoned = {
  ...
  it("D19A-011 PURE_POLICY_SEAM Vortox plus POISONED remains Vortox-constrained false", () => {
    const result = resolveDreamerV2Candidates({
      roleCatalogSnapshot: catalog([dreamer, flowergirl, witch, vortox]), roleCatalogSignature: "signed",
      targetTruth: targetTruth(dreamer),
  ...
  it("D19A-012 candidate shortage fails closed", () => {
    expect(resolveDreamerV2Candidates({
  ...
  it("D19A-013 PURE_POLICY_SEAM impaired information falls back to truth when no false native candidate exists", () => {
    const result = resolveDreamerV2Candidates({
      roleCatalogSnapshot: catalog([dreamer, witch]), roleCatalogSignature: "signed", targetTruth: targetTruth(dreamer),
      sourceEffectiveness: { kind: "KNOWN_DRUNK", representedImpairments: [{
        impairmentId: abilityImpairmentId("drunk"), impairmentKind: "DRUNK", sourceKind: "PHILOSOPHER_CHOSEN_DUPLICATE",
  ...
  it("D19A-014 HOSTILE_REPLAY_REJECTION rejects missing Vortox tenure", () => {
  ...
  it("D19A-015 HOSTILE_REPLAY_REJECTION rejects conflicting current Vortox identities", () => {
    expect(() => resolveDreamerV2VortoxConstraintForInternalValidation({
  ...
  it("D19A-016 PURE_POLICY_SEAM returns exact impaired-current-Vortox constraint", () => {
  ...
  it("D19A-017 rejects hostile Dreamer V2 canonical IDs and preserves formatter/parser round trips", () => {
    ...
    expect(() => formatDreamerV2TargetChoiceId(gainedTaskId)).toThrow(); // 2
    expect(parseDreamerV2DeliveryId(baseDeliveryId)).toMatchObject({ valid: true, taskId: baseTaskId }); // 3
    expect(() => formatDreamerV2DeliveryId(gainedTaskId)).toThrow(); // 4
    ...
    expect(validateDreamerTargetChosenV2PayloadShape({ ...targetPayload, targetChoiceId: `dreamer-target-choice-v2:${gainedTaskId}` })).toMatchObject({ valid: false }); // 5
    expect(validateDreamerInformationDeliveredV2PayloadShape({ ...deliveryPayload, deliveryId: `dreamer-delivery-v2:${gainedTaskId}` })).toMatchObject({ valid: false }); // 6
    ...
    expect(parseDreamerV2DeliveryId(`dreamer-delivery-v2:${gainedTaskId}`)).toMatchObject({ valid: false });
    ...
    expect(parseFirstNightActionOpportunityId(actionOpportunityId("first-night-v2:PHILOSOPHER_GAINED:DREAMER_ACTION:seat-09:from-dreamer:opportunity-100")))
      .toMatchObject({ valid: false }); // 18
  ...
  it("D19A-018 STRUCTURAL_VALIDATION rejects opportunity role-segment mismatch", () => {
  ...
  it("D19A-019 validates complete base Dreamer V2 source-contract identity and exact shape", () => {
  ...
  it("D19A-020 STRUCTURAL_VALIDATION rejects extra payload keys", () => {
  ...
  it("D19A-021 STRUCTURAL_VALIDATION rejects extra fields across opportunity, choice, and delivery validators", () => {
  ...
  it("D19A-022 HOSTILE_SOURCE_CONTINUITY_SEAM rejects fabricated role, seat, and tenure discontinuity", () => {
  ...
  it("D19A-023 PURE_POLICY_SEAM classifies POISONED false as SOURCE_POISONING", () => {
  ...
  it("D19A-024 CROSS_PLATFORM_CI freezes canonical IDs, ordering, and evidence ranks", () => {
  ...
  it("D19A-025 PURE_POLICY_SEAM alignment-only change does not change target truth", () => {
  ...
  it("D19A-026 PURE_POLICY_SEAM keeps an in-play false role legal", () => {
    const result = resolveDreamerV2Candidates({
  ...
  it("D19A-027 HOSTILE_REPLAY_REJECTION rejects death-capable alive evidence", () => {
  ...
  it("D19A-028 STRUCTURAL_VALIDATION preserves legacy evidence ranks and appends V2 ranks 17 and 18", () => {
  ...
  it("D19A-029 rejects mixed Dreamer V1/V2 state with same-source or same-delivery duplication", () => {
  ...
  it("D19A-030 CROSS_PLATFORM_CI rejects noncanonical opportunity index 010", () => {
    expect(parseFirstNightActionOpportunityId(actionOpportunityId(
      "first-night-v2:DREAMER_ACTION:BASE:seat-01:opportunity-010"
    ))).toMatchObject({ valid: false });
  });

  it("D19A-031 STRUCTURAL_VALIDATION preserves the accepted base task grammar only", () => {
    expect(parseCanonicalDreamerV2TaskId(baseTaskId)).toMatchObject({ valid: true, sourceKind: "BASE" });
    expect(parseCanonicalDreamerV2TaskId(
      "first-night-v2:PHILOSOPHER_GAINED:DREAMER_ACTION:seat-09:from-dreamer"
    )).toMatchObject({ valid: false });
  });

  it("D19A-032 PURE_POLICY_SEAM resolves an unimpaired base source as effective", () => {
    expect(resolveDreamerV2SourceEffectivenessForInternalValidation({
      sourceContract: baseSourceContract,
      abilityImpairments: { impairments: [] }
    })).toStrictEqual({ kind: "EFFECTIVE", representedImpairments: [] });
  });

  it("D19A-033 PURE_POLICY_SEAM resolves a canonical base source poison", () => {
    const impairment = {
      impairmentId: abilityImpairmentId("d19a-033"), kind: "POISONED" as const,
      sourceKind: "SNAKE_CHARMER_DEMON_HIT" as const, sourcePlayerId: playerId("snake"),
      affectedPlayerId: baseSourceContract.sourcePlayerId, affectedSeatNumber: baseSourceContract.sourceSeatNumber,
      affectedRole: dreamer, sourceCharacterStateRevision: 1
    };
    expect(resolveDreamerV2SourceEffectivenessForInternalValidation({
      sourceContract: baseSourceContract,
      abilityImpairments: { impairments: [impairment] }
    })).toMatchObject({ kind: "KNOWN_POISONED", representedImpairments: [{ impairmentId: impairment.impairmentId }] });
  });

  it("D19A-034 STRUCTURAL_VALIDATION rejects every gained Dreamer source contract", () => {
    expect(validateDreamerV2SourceContractShapeForInternalUse({
      ...baseSourceContract,
      kind: "PHILOSOPHER_GAINED_DREAMER_V2"
    })).toMatchObject({ valid: false });
  });

  it("D19A-035 PURE_POLICY_SEAM freezes the normal terminal ledger classification", () => {
    expect(classifyDreamerV2TerminalOutcomeForInternalValidation("TARGET_INCLUDED", "RULE_CORRECT"))
      .toStrictEqual({ outcomeStatus: "NORMAL", causeKind: "NO_OTHER_CHARACTER_ABILITY", causedByAnotherCharacterAbility: false });
  });

  it("D19A-036 PURE_POLICY_SEAM freezes the drunk-false terminal ledger classification", () => {
    expect(classifyDreamerV2TerminalOutcomeForInternalValidation("TARGET_EXCLUDED", "DETERMINISTIC_FALSE_WITH_KNOWN_DRUNKENNESS"))
      .toStrictEqual({ outcomeStatus: "ABNORMAL", causeKind: "SOURCE_DRUNKENNESS", causedByAnotherCharacterAbility: true });
  });

  it("D19A-037 PURE_POLICY_SEAM freezes the Vortox-false terminal ledger classification", () => {
    expect(classifyDreamerV2TerminalOutcomeForInternalValidation("TARGET_EXCLUDED", "VORTOX_CONSTRAINED_FALSE"))
      .toStrictEqual({ outcomeStatus: "ABNORMAL", causeKind: "VORTOX_FALSE_INFORMATION", causedByAnotherCharacterAbility: true });
  });

  it("D19A-038 STRUCTURAL_VALIDATION keeps V2 opportunity source identity base-only", () => {
    expect(validateFirstNightActionOpportunityStateShape({ opportunities: [{
      opportunitySchemaVersion: "dreamer-first-night-action-v2", nightNumber: 1,
      opportunityId: formatBaseDreamerV2ActionOpportunityId({ seatNumber: seatNumber(1) }),
      opportunityKind: "DREAMER_FIRST_NIGHT_ACTION", opportunityStatus: "OPEN", taskId: baseTaskId,
      taskType: "DREAMER_ACTION", sourcePlayerId: baseSourceContract.sourcePlayerId,
      sourceSeatNumber: baseSourceContract.sourceSeatNumber, sourceRole: dreamer, sourceCharacterStateRevision: 1,
      sourceContract: baseSourceContract,
      visibility: { visibilitySchemaVersion: "dreamer-first-night-action-v2",
        resolutionCapabilityVersion: "dreamer-first-night-resolution-capability-v2", canChooseTarget: true,
        supportedDecisionKinds: ["CHOOSE_PLAYER"], targetSchema: "OTHER_NON_TRAVELLER_MODELED_PLAYER" }
    }] })).toStrictEqual({ valid: true });
  });

  it("D19A-039 STRUCTURAL_VALIDATION rejects sparse V2 target choice collections", () => {
    const sparse = Array(1);
    expect(validateDreamerTargetChoiceSetV1V2({ choices: sparse })).toMatchObject({ valid: false });
  });

  it("D19A-040 STRUCTURAL_VALIDATION rejects sparse V2 delivery collections", () => {
    const sparse = Array(1);
    expect(validateDreamerInformationSetV1V2({ deliveries: sparse })).toMatchObject({ valid: false });
  });

  it("D19A-041 CROSS_PLATFORM_CI preserves legacy and V2 evidence rank ordering", () => {
    expect(abilityOutcomeEvidenceRankForInternalValidation("SOURCE_EVENT")).toBe(0);
    expect(abilityOutcomeEvidenceRankForInternalValidation("DREAMER_V2_ACTION_OPPORTUNITY")).toBe(17);
    expect(abilityOutcomeEvidenceRankForInternalValidation("DREAMER_V2_DELIVERY")).toBe(18);
  });

...
export type MathematicianDeliveryEvidence = { readonly kind: "MATHEMATICIAN_DELIVERY"; readonly deliveryId: string; readonly taskId: ScheduledTaskId; readonly sourcePlayerId: PlayerId; readonly trueCount: number; readonly selectedCount: number; readonly terminalEventId: EventId };
...
export const validateAbilityOutcomeEvidenceReferenceShape = (value: unknown): OutcomeLedgerValidationResult => {
  try {
    ...
    switch(e.kind){
      ...
      case "MATHEMATICIAN_DELIVERY":valid=id(e.deliveryId)&&id(e.taskId)&&id(e.sourcePlayerId)&&typeof e.trueCount==="number"&&Number.isSafeInteger(e.trueCount)&&e.trueCount>=0&&e.trueCount<=11&&typeof e.selectedCount==="number"&&Number.isSafeInteger(e.selectedCount)&&e.selectedCount>=0&&e.selectedCount<=11&&id(e.terminalEventId);break;
    }
...
export const validateFirstNightAbilityOutcomeFactShape = (value: unknown): OutcomeLedgerValidationResult => {
  try {
    ...
    const mathematician=kinds("MATHEMATICIAN_DELIVERY")[0] as MathematicianDeliveryEvidence|undefined;if(mathematician!==undefined){const correct=mathematician.trueCount===mathematician.selectedCount;if((correct&&(fact.outcomeStatus!=="NORMAL"||fact.causeKind!=="NO_OTHER_CHARACTER_ABILITY"||fact.causedByAnotherCharacterAbility))||(!correct&&(fact.outcomeStatus!=="ABNORMAL"||!fact.causedByAnotherCharacterAbility||!(fact.causeKind==="SOURCE_DRUNKENNESS"||fact.causeKind==="SOURCE_POISONING"||fact.causeKind==="VORTOX_FALSE_INFORMATION"))))setValid=false;}
...
export const deriveFirstNightAbilityOutcomeFact=(input:{readonly stateBefore:GameState;readonly event:AnyDomainEventEnvelope}):FirstNightAbilityOutcomeFact|undefined=>{
  ...
  if(input.event.eventType==="MathematicianInformationDelivered"&&plain(p.vortoxConstraint)&&p.vortoxConstraint.kind==="VORTOX_FALSE_REQUIRED"&&plain(p.vortoxConstraint.vortoxRoleTenure))evidence.push(roleEvidenceFor(input.stateBefore,p.vortoxConstraint.vortoxPlayerId as PlayerId,revision),tenureEvidenceFor(input.stateBefore,p.vortoxConstraint.vortoxRoleTenure.roleTenureId as RoleTenureId,revision));
  ...
  else if(input.event.eventType==="MathematicianInformationDelivered") evidence.push({kind:"MATHEMATICIAN_DELIVERY",deliveryId:p.deliveryId as string,taskId:task.taskId,sourcePlayerId:source.playerId,trueCount:p.trueCount as number,selectedCount:p.selectedCount as number,terminalEventId:input.event.eventId});

  ...
  try {
    ...
    switch(e.kind){
      ...
      case "ACTION_OPPORTUNITY": {
        const gainedDreamerV4 = e.opportunityKind === "DREAMER_FIRST_NIGHT_ACTION_V4";
        const parsedOpportunity = gainedDreamerV4
          ? { valid: /^first-night-v2:PHILOSOPHER_GAINED:DREAMER_ACTION:seat-(0[1-9]|1[0-2]):from-dreamer:opportunity-01$/.test(e.opportunityId), taskType: "DREAMER_ACTION", seatNumber: e.sourceSeatNumber }
          : e.opportunityKind === "DREAMER_FIRST_NIGHT_ACTION_V2" ||
        ...
        valid=e.opportunityVersion==="first-night-action-opportunity-v1"&&e.nightNumber===1&&parsedOpportunity.valid&&
          ["PHILOSOPHER_FIRST_NIGHT_ACTION","SNAKE_CHARMER_FIRST_NIGHT_ACTION","WITCH_FIRST_NIGHT_ACTION","CERENOVUS_FIRST_NIGHT_ACTION","DREAMER_FIRST_NIGHT_ACTION","DREAMER_FIRST_NIGHT_ACTION_V2","DREAMER_FIRST_NIGHT_ACTION_V3","DREAMER_FIRST_NIGHT_ACTION_V4","SEAMSTRESS_FIRST_NIGHT_ACTION"].includes(e.opportunityKind)&&
  ...
  try {
    ...
    const requireOpportunity=():boolean=>terminalOpportunity!==undefined&&opportunities.filter((entry)=>entry.taskId===fact.abilityTaskId).length===1&&
      terminalOpportunity.taskType===tasks[0]?.taskType&&terminalOpportunity.opportunityKind===(
        tasks[0].taskType==="DREAMER_ACTION"&&(terminalOpportunity.opportunityKind==="DREAMER_FIRST_NIGHT_ACTION_V3"||terminalOpportunity.opportunityKind==="DREAMER_FIRST_NIGHT_ACTION_V4")
          ? terminalOpportunity.opportunityKind
    ...
    if(sourceEvent.eventType==="DreamerInformationDelivered"&&fact.causeKind==="VORTOX_FALSE_INFORMATION")for(const entry of kinds("ROLE_TENURE") as RoleTenureEvidence[]){const parsed=parseRoleTenureId(entry.roleTenureId);if(!parsed.valid||parsed.seatNumber!==entry.seatNumber||parsed.roleId!==entry.roleId||parsed.acquiredCharacterStateRevision!==entry.acquiredCharacterStateRevision)setValid=false;}
...
const validateCanonicalDrunkFangGuDreamerDelivery=(state:GameState,delivery:DreamerInformationDeliveredPayloadV7):void=>{
  const opportunity=state.firstNightActionOpportunities?.opportunities.find((entry)=>entry.opportunityId===delivery.opportunityId);
  if(opportunity===undefined||!isDreamerActionOpportunityV3(opportunity)||state.firstNightTaskPlan===undefined||
    state.firstNightActionOpportunities===undefined||state.currentCharacterState===undefined||state.setup===undefined||
    state.seamstressRoleTenureState===undefined)throw new DomainError("InvalidFirstNightAbilityOutcomeEvidence","V7 Dreamer delivery requires complete canonical source state");
  const capability=resolveBaseDreamerV2NormalCapability({opportunity,firstNightTaskPlan:state.firstNightTaskPlan,
    firstNightTaskProgress:state.firstNightTaskProgress,firstNightActionOpportunities:state.firstNightActionOpportunities,
    currentCharacterState:state.currentCharacterState,setup:state.setup,roleTenures:state.seamstressRoleTenureState,
    abilityImpairments:state.abilityImpairments});
  if(capability.kind!=="CANONICAL_DRUNK_SOURCE_FANG_GU_APPARENT_INFORMATION_SUPPORTED")
    throw new DomainError("InvalidFirstNightAbilityOutcomeFact","V7 Dreamer delivery requires canonical-drunk Fang Gu capability");
  const targetChoice:DreamerTargetChosenPayloadV2={rulesBaselineVersion:delivery.rulesBaselineVersion,
    targetSchemaVersion:DREAMER_TARGET_CHOSEN_V2_SCHEMA_VERSION,nightNumber:1,taskId:delivery.taskId,
    taskType:"DREAMER_ACTION",opportunityId:delivery.opportunityId,opportunitySchemaVersion:delivery.opportunitySchemaVersion,
    decisionKind:"CHOOSE_PLAYER",sourcePlayerId:delivery.sourcePlayerId,sourceSeatNumber:delivery.sourceSeatNumber,
    sourceRole:opportunity.sourceRole,sourceCharacterStateRevision:delivery.sourceCharacterStateRevision,
    sourceContract:delivery.sourceContract,targetPlayerId:delivery.targetPlayerId,targetSeatNumber:delivery.targetSeatNumber};
  const expected=createDreamerCanonicalDrunkApparentInformationDeliveredPayload({rulesBaselineVersion:delivery.rulesBaselineVersion,
    targetChoice,setup:state.setup,currentCharacterState:state.currentCharacterState,capability});
  if(!sameDreamerInformationDelivery(delivery,expected))
    throw new DomainError("InvalidFirstNightAbilityOutcomeFact","V7 Dreamer delivery must equal its canonical apparent pair");
};
const validateCanonicalPhilosopherGainedDreamerDelivery=(state:GameState,delivery:DreamerInformationDeliveredPayloadV5|DreamerInformationDeliveredPayloadV6):void=>{
  const opportunity=state.firstNightActionOpportunities?.opportunities.find((entry)=>entry.opportunityId===delivery.opportunityId);
  if(opportunity===undefined||!isDreamerActionOpportunityV4(opportunity)||state.currentCharacterState===undefined||state.setup===undefined||
    state.seamstressRoleTenureState===undefined)throw new DomainError("InvalidFirstNightAbilityOutcomeEvidence","Gained Dreamer delivery requires complete canonical source state");
  const capability=resolvePhilosopherGainedDreamerCapability({opportunity,currentCharacterState:state.currentCharacterState,
    setup:state.setup,roleTenures:state.seamstressRoleTenureState,abilityImpairments:state.abilityImpairments});
  if((delivery.deliverySchemaVersion===DREAMER_INFORMATION_DELIVERED_V5_SCHEMA_VERSION&&capability.kind!=="NORMAL_INFORMATION_SUPPORTED")||
    (delivery.deliverySchemaVersion===DREAMER_INFORMATION_DELIVERED_V6_SCHEMA_VERSION&&capability.kind!=="VORTOX_FORCED_FALSE_INFORMATION_SUPPORTED"))
    throw new DomainError("InvalidFirstNightAbilityOutcomeFact","Gained Dreamer delivery requires its matching canonical capability");
  const targetChoice={rulesBaselineVersion:delivery.rulesBaselineVersion,targetSchemaVersion:"dreamer-target-chosen-v3" as const,
    nightNumber:1 as const,taskId:delivery.taskId,taskType:"DREAMER_ACTION" as const,opportunityId:delivery.opportunityId,
    opportunitySchemaVersion:delivery.opportunitySchemaVersion,decisionKind:"CHOOSE_PLAYER" as const,
    sourcePlayerId:delivery.sourcePlayerId,sourceSeatNumber:delivery.sourceSeatNumber,sourceRole:opportunity.sourceRole,
    sourceCharacterStateRevision:delivery.sourceCharacterStateRevision,evaluatedCharacterStateRevision:delivery.evaluatedCharacterStateRevision,
    sourceContract:delivery.sourceContract,abilityRoleId:"dreamer" as const,targetPlayerId:delivery.targetPlayerId,targetSeatNumber:delivery.targetSeatNumber};
  const expected=createPhilosopherGainedDreamerInformationDeliveredPayload({rulesBaselineVersion:delivery.rulesBaselineVersion,
    targetChoice,setup:state.setup,currentCharacterState:state.currentCharacterState,
    capability:capability as Extract<typeof capability,{readonly kind:"NORMAL_INFORMATION_SUPPORTED"|"VORTOX_FORCED_FALSE_INFORMATION_SUPPORTED"}>});
  if(!sameDreamerInformationDelivery(delivery,expected))throw new DomainError("InvalidFirstNightAbilityOutcomeFact","Gained Dreamer delivery must equal its canonical information pair");
};
...
const opportunityEvidenceFor=(state:GameState,opportunityIdValue:unknown,task:ScheduledTask,source:{playerId:PlayerId;seatNumber:SeatNumber;roleId:RoleId},eventType:TerminalAbilityOutcomeEventType,abilityInstance:FirstNightAbilityInstanceProvenance,expectedDreamerV3=false):ActionOpportunityEvidence=>{
  ...
  const gainedDreamerV4=expectedDreamerV3&&abilityInstance.kind==="PHILOSOPHER_GAINED_TASK_V2";
  const expectedKind=gainedDreamerV4?"DREAMER_FIRST_NIGHT_ACTION_V4":expectedDreamerV3?"DREAMER_FIRST_NIGHT_ACTION_V3":opportunityKindForTask(task.taskType);const parsed=opportunity===undefined||gainedDreamerV4?undefined:(expectedDreamerV3?parseBaseDreamerV2FirstNightActionOpportunityId(opportunity.opportunityId):parseFirstNightActionOpportunityId(opportunity.opportunityId));
  const parsedMatches=gainedDreamerV4?opportunity?.opportunityId===`${task.taskId}:opportunity-01`:
    parsed?.valid===true&&parsed.taskType===task.taskType&&parsed.seatNumber===source.seatNumber;
  ...
  if(matches.length!==1||opportunity===undefined||expectedKind===undefined||expectedRole===undefined||
    !parsedMatches||
    opportunity.opportunityStatus!==expectedPreTerminalOpportunityStatus(eventType)||opportunity.opportunityKind!==expectedKind||
    opportunity.taskId!==task.taskId||opportunity.taskType!==task.taskType||
...
export const deriveFirstNightAbilityOutcomeFact=(input:{readonly stateBefore:GameState;readonly event:AnyDomainEventEnvelope}):FirstNightAbilityOutcomeFact|undefined=>{
  ...
  if(input.event.eventType==="WitchIneffectiveResolved"){status="PENDING_TRIGGER";cause=p.sourceImpairmentKind==="POISONED"?"SOURCE_POISONING":"SOURCE_DRUNKENNESS";caused=true;}
    ...
    const target=input.stateBefore.currentCharacterState?.entries.find((entry)=>entry.playerId===p.targetPlayerId&&entry.seatNumber===p.targetSeatNumber);const good=p.goodRole as Record<string,unknown>;const evil=p.evilRole as Record<string,unknown>;const containsTruth=target!==undefined&&(target.role.roleId===good.roleId||target.role.roleId===evil.roleId);const dreamerV2=p.deliverySchemaVersion==="dreamer-information-delivered-v2";const dreamerV3=p.deliverySchemaVersion===DREAMER_INFORMATION_DELIVERED_V3_SCHEMA_VERSION;const dreamerV4=p.deliverySchemaVersion===DREAMER_INFORMATION_DELIVERED_V4_SCHEMA_VERSION;const dreamerV5=p.deliverySchemaVersion===DREAMER_INFORMATION_DELIVERED_V5_SCHEMA_VERSION;const dreamerV6=p.deliverySchemaVersion===DREAMER_INFORMATION_DELIVERED_V6_SCHEMA_VERSION;const dreamerV7=p.deliverySchemaVersion===DREAMER_INFORMATION_DELIVERED_V7_SCHEMA_VERSION;
    if(dreamerV7){validateCanonicalDrunkFangGuDreamerDelivery(input.stateBefore,input.event.payload as DreamerInformationDeliveredPayloadV7);if(target===undefined||p.informationReliability.kind!=="CANONICAL_SOURCE_DRUNK_APPARENT_INFORMATION")throw new DomainError("InvalidFirstNightAbilityOutcomeFact","V7 Dreamer delivery requires canonical apparent information");if(!containsTruth){status="ABNORMAL";cause="SOURCE_DRUNKENNESS";caused=true;}}
    else if(dreamerV5||dreamerV6){validateCanonicalPhilosopherGainedDreamerDelivery(input.stateBefore,input.event.payload as DreamerInformationDeliveredPayloadV5|DreamerInformationDeliveredPayloadV6);if(target===undefined||(dreamerV5?!containsTruth:containsTruth))throw new DomainError("InvalidFirstNightAbilityOutcomeFact","Gained Dreamer delivery truth relation is invalid");if(dreamerV6){status="ABNORMAL";cause="VORTOX_FALSE_INFORMATION";caused=true;}}
  ...
  if(input.event.eventType==="DreamerInformationDelivered"&&(p.deliverySchemaVersion===DREAMER_INFORMATION_DELIVERED_V5_SCHEMA_VERSION||p.deliverySchemaVersion===DREAMER_INFORMATION_DELIVERED_V6_SCHEMA_VERSION)){
    const sourceContract=p.sourceContract;if(!plain(sourceContract)||sourceContract.sourceContractVersion!=="dreamer-philosopher-gained-source-contract-v1"||sourceContract.taskPlanVersion!=="first-night-task-plan-v2"||sourceContract.taskId!==task.taskId||sourceContract.sourcePlayerId!==source.playerId||sourceContract.sourceSeatNumber!==source.seatNumber||sourceContract.sourceRoleId!=="philosopher"||sourceContract.chosenRoleId!=="dreamer"||!nonEmpty(sourceContract.sourceRoleTenureId)||sourceContract.sourceAbilityInstanceId!==instance.abilityInstanceId)throw new DomainError("InvalidFirstNightAbilityOutcomeEvidence","Gained Dreamer source contract must bind task, Philosopher tenure, source, and gained ability instance");
    evidence.push(tenureEvidenceFor(input.stateBefore,sourceContract.sourceRoleTenureId as RoleTenureId,revision));
  }
  ...
  if(input.event.eventType==="DreamerInformationDelivered"&&p.deliverySchemaVersion===DREAMER_INFORMATION_DELIVERED_V7_SCHEMA_VERSION&&plain(p.sourceImpairment))evidence.push(impairmentEvidenceFor(input.stateBefore,p.sourceImpairment.impairmentId,source));
  if(input.event.eventType==="DreamerInformationDelivered"&&p.deliverySchemaVersion!=="dreamer-information-delivered-v2"&&p.deliverySchemaVersion!==DREAMER_INFORMATION_DELIVERED_V5_SCHEMA_VERSION&&p.deliverySchemaVersion!==DREAMER_INFORMATION_DELIVERED_V7_SCHEMA_VERSION){const vortox=historicalVortoxApplicability(input.stateBefore,revision);if(vortox.kind==="PROVEN")evidence.push(roleEvidenceFor(input.stateBefore,vortox.playerId,revision),tenureEvidenceFor(input.stateBefore,vortox.tenureId,revision));else if(vortox.kind==="UNRESOLVED"&&vortox.playerId!==undefined)evidence.push(roleEvidenceFor(input.stateBefore,vortox.playerId,revision));}

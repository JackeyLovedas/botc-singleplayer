...
export type TerminalAbilityOutcomeEventType =
  ...
  | "DreamerInformationDelivered" | "DreamerInformationDeliveredV2" | "SeamstressInformationDelivered"
...
export type DreamerV2ActionOpportunityEvidence = { readonly kind: "DREAMER_V2_ACTION_OPPORTUNITY"; readonly opportunityVersion: "first-night-action-opportunity-v2"; readonly opportunitySchemaVersion: "dreamer-first-night-action-v2"; readonly nightNumber: 1; readonly opportunityId: ActionOpportunityId; readonly opportunityKind: "DREAMER_FIRST_NIGHT_ACTION"; readonly opportunityStatus: "OPEN"; readonly taskId: ScheduledTaskId; readonly taskType: "DREAMER_ACTION"; readonly sourceKind: "BASE_DREAMER_V2" | "PHILOSOPHER_GAINED_DREAMER_V2"; readonly sourcePlayerId: PlayerId; readonly sourceSeatNumber: SeatNumber; readonly sourceRole: RoleSetupSnapshot; readonly sourceCharacterStateRevision: number; readonly abilityInstanceId: FirstNightAbilityInstanceId };
export type DreamerV2DeliveryEvidence = { readonly kind: "DREAMER_V2_DELIVERY"; readonly deliveryId: DreamerV2DeliveryId; readonly taskId: ScheduledTaskId; readonly opportunityId: ActionOpportunityId; readonly sourcePlayerId: PlayerId; readonly abilityInstanceId: FirstNightAbilityInstanceId; readonly targetPlayerId: PlayerId; readonly targetTrueRoleId: RoleId; readonly deliveredGoodRoleId: RoleId; readonly deliveredEvilRoleId: RoleId; readonly candidateDomainVersion: "dreamer-candidate-domain-v2"; readonly roleCatalogSignature: string; readonly informationReliability: DreamerV2InformationReliability; readonly vortoxConstraintKind: DreamerV2VortoxConstraint["kind"]; readonly terminalEventId: EventId };
...
const primary = (e: AbilityOutcomeEvidenceReference): string => {
  switch(e.kind){
    ...
    case "CLOCKMAKER_DELIVERY":return e.deliveryId;case "DREAMER_DELIVERY":return e.terminalEventId;case "SEAMSTRESS_DELIVERY":return e.terminalEventId;case "MATHEMATICIAN_DELIVERY":return e.deliveryId;case "DREAMER_V2_ACTION_OPPORTUNITY":return e.opportunityId;case "DREAMER_V2_DELIVERY":return e.deliveryId;
  }
};
...
export const validateAbilityOutcomeEvidenceReferenceShape = (value: unknown): OutcomeLedgerValidationResult => {
  try {
    ...
    switch(e.kind){
      ...
      case "DREAMER_V2_ACTION_OPPORTUNITY": {
        const parsed=parseFirstNightActionOpportunityId(e.opportunityId);
        valid=e.opportunityVersion==="first-night-action-opportunity-v2"&&e.opportunitySchemaVersion==="dreamer-first-night-action-v2"&&
          e.nightNumber===1&&e.opportunityKind==="DREAMER_FIRST_NIGHT_ACTION"&&e.opportunityStatus==="OPEN"&&
          e.taskType==="DREAMER_ACTION"&&parsed.valid&&parsed.generation==="V2"&&parsed.taskType===e.taskType&&
          parsed.seatNumber===e.sourceSeatNumber&&["BASE_DREAMER_V2","PHILOSOPHER_GAINED_DREAMER_V2"].includes(e.sourceKind)&&
          id(e.sourcePlayerId)&&seat(e.sourceSeatNumber)&&hasExactRoleSetupSnapshotShape(e.sourceRole)&&
          positive(e.sourceCharacterStateRevision)&&parseFirstNightAbilityInstanceId(e.abilityInstanceId).valid;
        break;
      }
      case "DREAMER_V2_DELIVERY":valid=id(e.deliveryId)&&id(e.taskId)&&id(e.opportunityId)&&id(e.sourcePlayerId)&&
        parseFirstNightAbilityInstanceId(e.abilityInstanceId).valid&&id(e.targetPlayerId)&&id(e.targetTrueRoleId)&&
        id(e.deliveredGoodRoleId)&&id(e.deliveredEvilRoleId)&&e.deliveredGoodRoleId!==e.deliveredEvilRoleId&&
        e.candidateDomainVersion==="dreamer-candidate-domain-v2"&&id(e.roleCatalogSignature)&&
        ["RULE_CORRECT","DETERMINISTIC_TRUE_WITH_KNOWN_DRUNKENNESS","DETERMINISTIC_FALSE_WITH_KNOWN_DRUNKENNESS","DETERMINISTIC_TRUE_WITH_KNOWN_POISONING","DETERMINISTIC_FALSE_WITH_KNOWN_POISONING","VORTOX_CONSTRAINED_FALSE"].includes(e.informationReliability)&&
        ["NONE_NO_CURRENT_VORTOX","NONE_CURRENT_VORTOX_KNOWN_IMPAIRED","VORTOX_FALSE_REQUIRED"].includes(e.vortoxConstraintKind)&&id(e.terminalEventId);break;
    }
...
const dreamerV2EvidenceSlot = (
  evidence: AbilityOutcomeEvidenceReference,
  delivery: DreamerV2DeliveryEvidence
): number => {
  switch(evidence.kind){
    case "SOURCE_EVENT":return 1;case "TASK":return 2;case "DREAMER_V2_ACTION_OPPORTUNITY":return 3;case "CHARACTER_STATE":return 4;
    case "PLAYER_ROLE_AT_REVISION":if(evidence.playerId===delivery.sourcePlayerId)return 5;if(evidence.playerId===delivery.targetPlayerId)return 7;if(evidence.roleId==="vortox")return 9;break;
    case "ROLE_TENURE":if(evidence.playerId===delivery.sourcePlayerId)return 6;if(evidence.roleId==="vortox")return 10;break;
    case "ABILITY_IMPAIRMENT":if(evidence.affectedPlayerId===delivery.sourcePlayerId)return 8;if(evidence.affectedRoleId==="vortox")return 11;break;
    case "ACTION_OPPORTUNITY":return 12;case "PHILOSOPHER_GRANT":return 13;case "FIRST_NIGHT_TASK_INSERTION":return 14;case "DREAMER_V2_DELIVERY":return 15;
    default:break;
  }
  throw new DomainError("InvalidFirstNightAbilityOutcomeEvidence","Dreamer V2 evidence does not fit the frozen 15-slot matrix");
};
export const canonicalizeAbilityOutcomeEvidenceReferences = (values: unknown): readonly AbilityOutcomeEvidenceReference[] => {
  ...
  const dreamerV2Deliveries=captured.filter((entry):entry is DreamerV2DeliveryEvidence=>entry.kind==="DREAMER_V2_DELIVERY");
  if(deliveries.length>0&&dreamerV2Deliveries.length>0)throw new DomainError("InvalidFirstNightAbilityOutcomeEvidence","One fact cannot mix Mathematician and Dreamer V2 deliveries");
  const sorted = dreamerV2Deliveries.length>0
    ?(()=>{if(dreamerV2Deliveries.length!==1)throw new DomainError("InvalidFirstNightAbilityOutcomeEvidence","Dreamer V2 evidence requires one delivery identity");const delivery=dreamerV2Deliveries[0]!;const slotted=captured.map((entry)=>({entry,slot:dreamerV2EvidenceSlot(entry,delivery)}));const seen=new Set<number>();for(const item of slotted){if(seen.has(item.slot))throw new DomainError("InvalidFirstNightAbilityOutcomeEvidence","Dreamer V2 evidence slots cannot repeat");seen.add(item.slot);}return slotted.sort((a,b)=>a.slot-b.slot).map((item)=>item.entry);})()
    : deliveries.length === 0
...
export const validateFirstNightAbilityOutcomeFactShape = (value: unknown): OutcomeLedgerValidationResult => {
  try {
    ...
    switch(sourceEvent.eventType){
      ...
      case "DreamerInformationDeliveredV2":setValid=roles.length>=2&&kinds("ROLE_TENURE").length>=1&&kinds("DREAMER_V2_ACTION_OPPORTUNITY").length===1&&kinds("DREAMER_V2_DELIVERY").length===1&&exactKinds(["SOURCE_EVENT","TASK","DREAMER_V2_ACTION_OPPORTUNITY","ACTION_OPPORTUNITY","CHARACTER_STATE","PLAYER_ROLE_AT_REVISION","ABILITY_IMPAIRMENT","ROLE_TENURE","PHILOSOPHER_GRANT","FIRST_NIGHT_TASK_INSERTION","DREAMER_V2_DELIVERY"]);break;
    ...
    const dreamerV2Opportunity=kinds("DREAMER_V2_ACTION_OPPORTUNITY")[0] as DreamerV2ActionOpportunityEvidence|undefined;
    const dreamerV2=kinds("DREAMER_V2_DELIVERY")[0] as DreamerV2DeliveryEvidence|undefined;
    if(dreamerV2!==undefined){const targetRole=roleOf(dreamerV2.targetPlayerId);const containsTruth=targetRole?.roleId===dreamerV2.targetTrueRoleId&&(dreamerV2.targetTrueRoleId===dreamerV2.deliveredGoodRoleId||dreamerV2.targetTrueRoleId===dreamerV2.deliveredEvilRoleId);if(dreamerV2Opportunity===undefined||dreamerV2Opportunity.taskId!==fact.abilityTaskId||dreamerV2Opportunity.opportunityId!==dreamerV2.opportunityId||dreamerV2Opportunity.sourcePlayerId!==fact.sourcePlayerId||dreamerV2Opportunity.abilityInstanceId!==fact.abilityInstance.abilityInstanceId||dreamerV2.taskId!==fact.abilityTaskId||dreamerV2.sourcePlayerId!==fact.sourcePlayerId||dreamerV2.abilityInstanceId!==fact.abilityInstance.abilityInstanceId||dreamerV2.terminalEventId!==fact.sourceEventId||targetRole===undefined||fact.outcomeStatus==="UNRESOLVED")setValid=false;if(fact.outcomeStatus==="NORMAL"&&(!containsTruth||fact.causeKind!=="NO_OTHER_CHARACTER_ABILITY"))setValid=false;if(fact.outcomeStatus==="ABNORMAL"&&containsTruth)setValid=false;if(dreamerV2.informationReliability==="VORTOX_CONSTRAINED_FALSE"&&(fact.outcomeStatus!=="ABNORMAL"||fact.causeKind!=="VORTOX_FALSE_INFORMATION"))setValid=false;}
...
const dreamerV2OpportunityEvidenceFor=(state:GameState,opportunityIdValue:unknown,task:ScheduledTask,source:{playerId:PlayerId;seatNumber:SeatNumber;roleId:RoleId},instance:FirstNightAbilityInstanceProvenance):DreamerV2ActionOpportunityEvidence=>{
  const matches=state.firstNightActionOpportunities?.opportunities.filter((entry)=>entry.opportunityId===opportunityIdValue)??[];const opportunity=matches[0];
  if(matches.length!==1||opportunity===undefined||!isDreamerActionOpportunityV2(opportunity)||opportunity.opportunityStatus!=="OPEN"||opportunity.taskId!==task.taskId||opportunity.sourcePlayerId!==source.playerId||opportunity.sourceSeatNumber!==source.seatNumber||opportunity.sourceContract.abilityInstance.abilityInstanceId!==instance.abilityInstanceId)throw new DomainError("InvalidFirstNightAbilityOutcomeEvidence","Dreamer V2 terminal requires one exact open V2 opportunity");
  return {kind:"DREAMER_V2_ACTION_OPPORTUNITY",opportunityVersion:"first-night-action-opportunity-v2",opportunitySchemaVersion:opportunity.opportunitySchemaVersion,nightNumber:1,opportunityId:opportunity.opportunityId,opportunityKind:"DREAMER_FIRST_NIGHT_ACTION",opportunityStatus:"OPEN",taskId:opportunity.taskId,taskType:"DREAMER_ACTION",sourceKind:opportunity.sourceContract.kind,sourcePlayerId:opportunity.sourcePlayerId,sourceSeatNumber:opportunity.sourceSeatNumber,sourceRole:clone(opportunity.sourceRole),sourceCharacterStateRevision:opportunity.sourceCharacterStateRevision,abilityInstanceId:instance.abilityInstanceId};
};
...
export const classifyDreamerV2TerminalOutcomeForInternalValidation=(
  truthOutcome:unknown,
  informationReliability:unknown
):{readonly outcomeStatus:AbilityOutcomeStatus;readonly causeKind:AbilityOutcomeCause;readonly causedByAnotherCharacterAbility:boolean}=>{
  const included=truthOutcome==="TARGET_INCLUDED";
  if(informationReliability==="RULE_CORRECT"||informationReliability==="DETERMINISTIC_TRUE_WITH_KNOWN_DRUNKENNESS"||informationReliability==="DETERMINISTIC_TRUE_WITH_KNOWN_POISONING"){
    if(!included)throw new DomainError("InvalidFirstNightAbilityOutcomeFact","Dreamer V2 true reliability requires target truth");
    return {outcomeStatus:"NORMAL",causeKind:"NO_OTHER_CHARACTER_ABILITY",causedByAnotherCharacterAbility:false};
  }
  if(included)throw new DomainError("InvalidFirstNightAbilityOutcomeFact","Dreamer V2 false reliability must exclude target truth");
  if(informationReliability==="DETERMINISTIC_FALSE_WITH_KNOWN_DRUNKENNESS")return {outcomeStatus:"ABNORMAL",causeKind:"SOURCE_DRUNKENNESS",causedByAnotherCharacterAbility:true};
  if(informationReliability==="DETERMINISTIC_FALSE_WITH_KNOWN_POISONING")return {outcomeStatus:"ABNORMAL",causeKind:"SOURCE_POISONING",causedByAnotherCharacterAbility:true};
  if(informationReliability==="VORTOX_CONSTRAINED_FALSE")return {outcomeStatus:"ABNORMAL",causeKind:"VORTOX_FALSE_INFORMATION",causedByAnotherCharacterAbility:true};
  throw new DomainError("InvalidFirstNightAbilityOutcomeFact","Dreamer V2 reliability is invalid");
};
export const deriveFirstNightAbilityOutcomeFact=(input:{readonly stateBefore:GameState;readonly event:AnyDomainEventEnvelope}):FirstNightAbilityOutcomeFact|undefined=>{
  ...
  else if(input.event.eventType==="DreamerInformationDeliveredV2"){
    const classification=classifyDreamerV2TerminalOutcomeForInternalValidation(p.truthOutcome,p.informationReliability);
    status=classification.outcomeStatus;cause=classification.causeKind;caused=classification.causedByAnotherCharacterAbility;
  }
  ...
  if(p.opportunityId!==undefined)evidence.push(input.event.eventType==="DreamerInformationDeliveredV2"?dreamerV2OpportunityEvidenceFor(input.stateBefore,p.opportunityId,task,source,instance):opportunityEvidenceFor(input.stateBefore,p.opportunityId,task,source,input.event.eventType as TerminalAbilityOutcomeEventType,instance));
  ...
  if(input.event.eventType==="DreamerInformationDeliveredV2"){
    const sourceContract=p.sourceContract;if(!plain(sourceContract)||!plain(sourceContract.sourceRoleTenure)||!nonEmpty(sourceContract.sourceRoleTenure.roleTenureId))throw new DomainError("InvalidFirstNightAbilityOutcomeEvidence","Dreamer V2 source requires one carried active tenure");
    evidence.push(tenureEvidenceFor(input.stateBefore,sourceContract.sourceRoleTenure.roleTenureId as RoleTenureId,revision));
  }
  ...
  if(input.event.eventType==="DreamerInformationDeliveredV2"&&plain(p.sourceEffectiveness)&&isDenseCanonicalArray(p.sourceEffectiveness.representedImpairments))for(const represented of p.sourceEffectiveness.representedImpairments)if(plain(represented))evidence.push(impairmentEvidenceFor(input.stateBefore,represented.impairmentId,source));
  ...
  if(input.event.eventType==="DreamerInformationDeliveredV2"&&plain(p.vortoxConstraint)&&(p.vortoxConstraint.kind==="VORTOX_FALSE_REQUIRED"||p.vortoxConstraint.kind==="NONE_CURRENT_VORTOX_KNOWN_IMPAIRED")&&plain(p.vortoxConstraint.vortoxRoleTenure)){
    const vortox={playerId:p.vortoxConstraint.vortoxPlayerId as PlayerId,seatNumber:p.vortoxConstraint.vortoxSeatNumber as SeatNumber,roleId:"vortox" as RoleId};evidence.push(roleEvidenceFor(input.stateBefore,vortox.playerId,revision),tenureEvidenceFor(input.stateBefore,p.vortoxConstraint.vortoxRoleTenure.roleTenureId as RoleTenureId,revision));
    const represented=p.vortoxConstraint.representedImpairments;if(p.vortoxConstraint.kind==="NONE_CURRENT_VORTOX_KNOWN_IMPAIRED"&&isDenseCanonicalArray(represented)&&plain(represented[0]))evidence.push(impairmentEvidenceFor(input.stateBefore,represented[0].impairmentId,vortox));
  }
  ...
  if(input.event.eventType.startsWith("SnakeCharmer")){
    ...
    const targetTruth=p.targetTruth as Record<string,unknown>;const selectedGood=p.selectedGoodRole as Record<string,unknown>;const selectedEvil=p.selectedEvilRole as Record<string,unknown>;const candidateDomain=p.candidateDomain as Record<string,unknown>;const sourceContract=p.sourceContract as Record<string,unknown>;
    evidence.push(roleEvidenceFor(input.stateBefore,targetTruth.targetPlayerId as PlayerId,revision),{kind:"DREAMER_V2_DELIVERY",deliveryId:p.deliveryId as DreamerV2DeliveryId,taskId:task.taskId,opportunityId:p.opportunityId as ActionOpportunityId,sourcePlayerId:source.playerId,abilityInstanceId:(sourceContract.abilityInstance as Record<string,unknown>).abilityInstanceId as FirstNightAbilityInstanceId,targetPlayerId:targetTruth.targetPlayerId as PlayerId,targetTrueRoleId:(targetTruth.targetTrueRole as Record<string,unknown>).roleId as RoleId,deliveredGoodRoleId:selectedGood.roleId as RoleId,deliveredEvilRoleId:selectedEvil.roleId as RoleId,candidateDomainVersion:candidateDomain.candidateDomainVersion as "dreamer-candidate-domain-v2",roleCatalogSignature:candidateDomain.roleCatalogSignature as string,informationReliability:p.informationReliability as DreamerV2InformationReliability,vortoxConstraintKind:(p.vortoxConstraint as Record<string,unknown>).kind as DreamerV2VortoxConstraint["kind"],terminalEventId:input.event.eventId});
  }

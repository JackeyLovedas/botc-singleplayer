...
const EXPECTED_TOTAL = Object.freeze({ ordinary: 1715, coverage: 1715, windows: 46 });
...
const CURRENT_ORDINARY_COUNTS = Object.freeze([207, 506, 465, 90, 52, 82, 26, 46, 241]);
const CURRENT_COVERAGE_COUNTS = Object.freeze([207, 506, 465, 90, 52, 73, 9, 26, 36, 10, 241]);
...
function selfTest() {
  ...
  check(EXPECTED_TOTAL.ordinary === 1715 && EXPECTED_TOTAL.coverage === 1715, "current totals");
  check(CURRENT_ORDINARY_COUNTS.reduce((sum, count) => sum + count, 0) === 1715, "current ordinary counts");
  check(CURRENT_COVERAGE_COUNTS.reduce((sum, count) => sum + count, 0) === 1715, "current coverage counts");

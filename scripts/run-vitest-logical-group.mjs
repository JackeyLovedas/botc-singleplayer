...
const EXPECTED_TOTAL = Object.freeze({ ordinary: 1719, coverage: 1719, windows: 46 });
...
const CURRENT_ORDINARY_COUNTS = Object.freeze([207, 509, 465, 91, 52, 82, 26, 46, 241]);
const CURRENT_COVERAGE_COUNTS = Object.freeze([207, 509, 465, 91, 52, 73, 9, 26, 36, 10, 241]);
...
function selfTest() {
  ...
  check(EXPECTED_TOTAL.ordinary === 1719 && EXPECTED_TOTAL.coverage === 1719, "current totals");
  check(CURRENT_ORDINARY_COUNTS.reduce((sum, count) => sum + count, 0) === 1719, "current ordinary counts");
  check(CURRENT_COVERAGE_COUNTS.reduce((sum, count) => sum + count, 0) === 1719, "current coverage counts");
